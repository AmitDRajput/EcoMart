...
namespace EcoMart.DataLayer
{
    public class DBWard
    {
        ...
        public DataTable GetOverviewData()
        {
            ...
            string strSql = "Select WardId, WardName from masterward order by WardName";
        ...
        public DataRow ReadDetailsByID(string Id)
        {
            ...
            if (Id != "")
            {
                string strSql = "Select * from masterward where WardId='{0}' ";
        ...
        private string GetDataForUniqueForAdd(string Name, string Id)
        {
            ...
            sQuery.AppendFormat("Select WardId from masterward where WardName='{0}'", Name);
            if (Id != "")
            {
                sQuery.AppendFormat(" AND WardId in ('{0}')", Id);
            }
        ...
        private string GetDataForUniqueForEdit(string Name, string Id)
        {
            ...
            if (Id != "")
            {
                sQuery.AppendFormat(" AND WardId not in ('{0}')", Id);
            }
            return sQuery.ToString();
        } 
        ...
        private string GetInsertQuery(string Id, string Name, string createdby, string createddate, string createdtime)
        {
            ...
            objQuery.Table = "masterward";
            objQuery.AddToQuery("WardId", Id);
            objQuery.AddToQuery("WardName", Name);
        ...
        private string GetUpdateQuery(string Id, string Name, string modifiedby, string modifydate, string modifytime)
        {
            Query objQuery = new Query();
            objQuery.Table = "masterward";
            objQuery.AddToQuery("WardId", Id, true);
            objQuery.AddToQuery("WardName", Name);
