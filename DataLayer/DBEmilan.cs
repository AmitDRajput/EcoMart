    ...
    class DBEmilan
    {
        ...
        public DataTable GetPurchaseOrdeerForOrderForToday(int firstNumber, int lastNumber)
        {
            ...
            string strSql = "select convert(a.ordernumber , char) as ordernumber,STR_TO_DATE( date_format(orderdate, '%d/%m/%y'),'%d/%m/%Y') as date,'RTTT00055' as supplierID,b.AIOCDACode as SupplierProductCode," +
                ...
                " a.DSLID,a.MasterID,c.AccName,d.Amount  from detailpurchaseorderstockist a inner join masterproduct b on a.ProductID = b.ProductID inner join masteraccount c on a.AccountID = c.AccountID inner join masterpurchaseorderstockist d on a.MasterID = d.ID where ordernumber >= "+ firstNumber +" && ordernumber <= "+ lastNumber +"  order by a.ordernumber"; //&& AIOCDACode is not null && AIOCDACode != '' order by ordernumber";
        ...
        public DataTable GetOrderDetails(string masterid)
        {
            ...
            string strSql = "select convert(a.ordernumber , char) as ordernumber,STR_TO_DATE( date_format(orderdate, '%d/%m/%y'),'%d/%m/%Y') as date,'RTTT00055' as supplierID,'' as SupplierProductCode," +
                ...
                " a.OrderQuantity as Quantity, a.SchemeQuantity as FreeQuantity, a.Narration as Remarks  from detailpurchaseorderstockist a inner join masterproduct b on a.ProductID = b.ProductID inner join masteraccount c on a.AccountID = c.AccountID  where masterID = '"+ masterid+"'"; //&& AIOCDACode is not null && AIOCDACode != '' order by ordernumber";
            dt = DBInterface.SelectDataTable(strSql);
            return dt;
        }
