...
namespace  EcoMart.DataLayer
{
    public class DBLicense
    {
        ...
        public bool InsertCreditorinMasterAccount(string macccode, string maccname, string maccaddress1, string maccaddress2, string marea, string macctelephone, string maccmobile, string mdlnno, string memail, string mgstno, string mjuri, string mcontactperson, string mcity)
        {
            bool bRetValue = false;
            string strSql = "insert into masteraccount ( AccCode,AccName,AccOpeningDebit,AccOpeningCredit,AccAddress1,AccAddress2,AccTelephone,MobileNumberForSMS,AccContactPerson,AccGroupID,AccVATTIN,AccDLN,AccEmailID) values ('" + macccode + "','" + maccname + "'," + 0 + "," + 0 + ",'"+ maccaddress1 + "','" + maccaddress2 + "','" + macctelephone + "','" + maccmobile + "','" + mcontactperson + "'," + 31 + ",'" + mgstno + "','" + mdlnno + "','" + memail + "')" ;
