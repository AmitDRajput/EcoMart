    ...
    public  class DBSaleList
    {
        ...
        public DataTable GetCreditNoteDataProfitPercentDay (string fromdate, string todate)
        {
            DataTable dt = null;
            {
                string strSql = "select c.VoucherDate, sum(a.Quantity * (a.ReturnRate/b.ProdLoosePack)) as AmountNet,sum(a.Quantity * ((a.PurchaseRate+(a.TradeRate*b.ProdVatPercent/100))/b.ProdLoosePack)) as AmountByPurchaseRate,sum(a.Quantity * ((a.ReturnRate - (a.PurchaseRate+(a.TradeRate*b.ProdVatPercent/100)))/b.ProdLoosePack)) as ProfitInRupees,sum((a.Quantity * ((a.SaleRate - (a.PurchaseRate+(a.TradeRate*b.ProdVatPercent/100)))/b.ProdLoosePack))/(a.Quantity * (a.salerate/b.ProdLoosePack))) as ProfitPercentBySaleRate,sum((a.Quantity * ((a.SaleRate - (a.PurchaseRate+(a.TradeRate*b.ProdVatPercent/100)))/b.ProdLoosePack))/(a.Quantity * ((a.purchaserate+(a.TradeRate*b.ProdVatPercent/100))/b.ProdLoosePack))) as ProfitPercentByPurchaseRate from detailcreditdebitnotestock a " +
                               " inner join vouchercreditdebitnote c on a.MasterID = c.CRDBID inner join masterproduct b on a.ProductID = b.ProductID  where c.voucherdate >= '" + fromdate + "' && c.voucherdate <= '" + todate + "' && c.voucherType = '" + FixAccounts.VoucherTypeForCreditNoteStock + "'   group by c.VoucherDate ";
        ...
        public DataTable ReadSaleDetailsForVATRegister(string  fromdate , string todate)
        {
            DataTable dt = null;
            {
                string strSql = "select b.ID,b.VoucherType, b.VoucherSubType," +
                                    ...
                                    " where voucherdate >= '" + fromdate + "' && voucherdate <= '" + todate + "' order by b.VoucherDate,b.vouchertype,b.vouchernumber ";                                 
        ...
        public DataTable ReadSaleDetailsForVATRegister(string fromdate, string todate, string accountID)
        {
            DataTable dt = null;
            {
                string strSql = "select b.ID,b.VoucherType, b.VoucherSubType," +
                                    ...
                                    " where voucherdate >= '" + fromdate + "' && voucherdate <= '" + todate + "' && AccountID = '"+ accountID +"'";
        ...
        public DataTable ReadSaleDetailsForVATRegisterTINDetail(string fromdate, string todate)
        {
            DataTable dt = null;
            {
                string strSql = "select b.ID,b.VoucherType,b.VoucherNumber,b.VoucherDate,(AmountVAT5Per + AmountVAT12point5Per) as TotalAmount,(b.VAT5Per+ b.VAT12Point5Per) as TotalVAT,(b.VAT5Per) as VAT5Per,(b.VAT12Point5Per) as VAT12Point5Per,b.AmountForZeroVAT,b.PatientName as AccName, b.PatientAddress1 as AccAddress1 ,c.AccVATTIN   from  vouchersale b  left outer join masteraccount c on  b.AccountID = c.AccountID " +
                                    " where voucherdate >= '" + fromdate + "' && voucherdate <= '" + todate + "' order by VoucherNumber";
                dt = DBInterface.SelectDataTable(strSql);
            }
            return dt;
        }
        ...
        public DataTable GetSaleDataForDoctorShowReport(string doctorid, string compID, string fromdate, string todate)
        {
            DataTable dt = null;
            {
                string strSql = "select a.MasterSaleID,a.ProductID,a.BatchNumber,a.SaleRate, " +
                                ...
                                "on A.MasterSaleID = B.ID  left outer join masteraccount c on b.AccountID = c.AccountID  inner join masterproduct d on a.ProductID = d.ProductID  where  b.DoctorID = '" + doctorid + "' && b.VoucherDate >= '" + fromdate + "' && b.Voucherdate <= '" + todate + "' && d.ProdCompID = '"+ compID +"' order by b.Vouchertype,b.VoucherNumber";
        ...
        public DataTable GetSaleDataForDoctor(string doctorid, string compid, string fromdate, string todate)
        {
            DataTable dt = null;
            {
                string strSql = "select a.MasterSaleID,a.ProductID,a.BatchNumber,a.SaleRate, " +
                                ...
                                "b.VoucherNumber,b.VoucherSubType,b.VoucherDate,b.PatientName,b.PatientAddress1,c.AccCode,d.ProductID, " +
                                "d.ProdName,d.ProdLoosePack,d.ProdPack,d.ProdCompShortName,d.ProdCompID from detailsale a  inner join vouchersale b " +
                                "on A.MasterSaleID = B.ID  left outer join masteraccount c on b.AccountID = c.AccountID  inner join masterproduct d on a.ProductID = d.ProductID  where  b.DoctorID = '" + doctorid + "' && d.ProdCompID = '"+ compid +"' && b.VoucherDate >= '" + fromdate + "' && b.Voucherdate <= '" + todate + "' order by b.Vouchertype,b.VoucherNumber";
        ...
        public DataTable GetOverviewDataForAllPartySummary(string fromdate, string todate)
        {
            ...
            string strSql = "Select a.voucherdate,a.vouchertype, a.AccountID , sum(a.AmountNet) as AmountNet,a.PatientName,a.PatientAddress1,b.AccName,b.AccAddress1 " +
                             "from vouchersale a inner join masteraccount b on a.AccountID = b.AccountId where a.AccountID != '" + FixAccounts.AccountCash + "' && a.AccountID != '" + FixAccounts.AccountPendingCashBills + "' && voucherdate >= '" + fromdate + "' &&  voucherdate <= '" + todate + "'  group by AccountID ,VoucherType  order by patientname ";
            dtable = DBInterface.SelectDataTable(strSql);
            return dtable;
        }
            ...
            {
                {
                    ...
                    string strSql = "select ID,VoucherType,countersalenumber as VoucherNumber,VoucherSubType, VoucherDate,PatientName as AccName, PatientAddress1 as AccAddress1, sum(AmountNet) as Amount, PatientAddress2 as AccAddress2 from vouchersale " +
                                    " where VoucherSubType != '"+ FixAccounts.SubTypeForRegularSale + "' && VoucherType = '" + FixAccounts.VoucherTypeForVoucherSale + "' && voucherdate = '" + fromdate + "' Group by countersalenumber order by vouchertype,VoucherNumber";
            ...
            {
                {
                    ...
                    string strSql = "select  b.ID  , b.VoucherType, " +
                                    ...
                                    "where b.VoucherSubType = '" + FixAccounts.SubTypeForRegularSale + "'  &&  b.voucherdate = '" + fromdate + "' order by b.VoucherNumber ";
            ...
            {
                {
                    string strSql = "select  b.ID  , b.VoucherType, " +
                                    ...
                                    "left outer join masteraccount a on b.AccountID = a.AccountID where (b.AccountID = '" + accID + "' || b.PatientID = '" + accID + "') && b.VoucherSubType = '" + saleSubType + "' && b.AmountBalance > 0   &&  b.voucherdate >= '" + fromdate + "'  && b.VoucherDate <= '" + todate + "' " +
                                   ...
                                   "left outer join masteraccount a on b.AccountID = a.AccountID where b.AccountID = '" + accID + "' &&  b.onAccountAmount > 0   &&  b.voucherdate >= '" + fromdate + "'  && b.VoucherDate <= '" + todate + "' order by AccName, accAddress1,VoucherDate";
                    dt = DBInterface.SelectDataTable(strSql);
                }
            }
            catch (Exception Ex)
            ...
            {
                {
                    string strSql = "select  b.ID  , b.VoucherType, " +
                                    ...
                                    "inner join masteraccount a on b.AccountID = a.AccountID where b.VoucherSubType = '" + saleSubType + "' && (b.AmountBalance > 0 || a.AccOpeningDebit > 0 || a.AccOpeningCredit > 0)  &&  b.voucherdate >= '" + fromdate + "'  && b.VoucherDate <= '" + todate + "'" +
                                    ...
                                    "inner join masteraccount a on b.AccountID = a.AccountID where b.VoucherSubType = '" + saleSubType + "' && (b.AmountBalance > 0 || a.AccOpeningDebit > 0 || a.AccOpeningCredit > 0)   &&  b.voucherdate >= '" + fromdate + "'  && b.VoucherDate <= '" + todate + "' " +
                                     ...
                                     " union select  b.cbID  as ID , b.VoucherType, " +
                                    "b.VoucherNumber,'',b.VoucherDate,b.AccountID,a.AccName,a.AccAddress1,0,0,'',0,0,0,b.onaccountAmount  from  vouchercashbankreceipt b " +
                                    "inner join masteraccount a on b.AccountID = a.AccountID where b.onAccountAmount > 0   &&  b.voucherdate >= '" + fromdate + "'  && b.VoucherDate <= '" + todate + "' order by AccName, accAddress1,VoucherDate";
                    dt = DBInterface.SelectDataTable(strSql);
                }
            }
            catch (Exception Ex)
            ...
            {
                {
                    string strSql = "select  b.ID  , b.VoucherType, " +
                                    ...
                                    "inner join masteraccount d on b.AccountID = d.AccountID  where (d.AccAreaID = '" + accID + "' ) && b.VoucherSubType = '" + saleSubType + "' && b.AmountBalance > 0   &&  b.voucherdate >= '" + fromdate + "'  && b.VoucherDate <= '" + todate + "' " +
                                    ...
                                    "inner join masteraccount d on b.AccountID = d.AccountID  where (d.AccAreaID = '" + accID + "' ) && b.VoucherSubType = '" + saleSubType + "' && b.AmountBalance > 0   &&  b.voucherdate >= '" + fromdate + "'  && b.VoucherDate <= '" + todate + "' order by AccName,AccAddress1";
            ...
            {
                {
                    string strSql = "select  b.ID  , b.VoucherType, " +
                                    ...
                                    "inner join masteraccount a on b.AccountID = a.AccountID where b.VoucherSubType = '" + saleSubType + "' && b.AmountBalance > 0   &&  b.voucherdate >= '" + fromdate + "'  && b.VoucherDate <= '" + todate + "' order by b.PatientName, b.PatientAddress1,b.VoucherDate";
                    dt = DBInterface.SelectDataTable(strSql);
                }
            }
            catch (Exception Ex)
            ...
            {
                {
                    string strSql = "select  b.ID  , b.VoucherType, " +
                                    ...
                                    "where (b.AccountID = '" + accID + "' || b.PatientID = '" + accID + "') &&  b.voucherdate >= '" + fromdate + "'  && b.VoucherDate <= '" + todate + "' "+
                                    ...
                                    "where (b.AccountID = '" + accID + "' || b.PatientID = '" + accID + "') &&  b.voucherdate >= '" + fromdate + "'  && b.VoucherDate <= '" + todate + "' order by VoucherDate,VoucherNumber";
        ...
        {
            DataTable dt = null;
            string strSql = "select a.ID,a.VoucherType,a.VoucherNumber,a.CounterSaleNumber,a.VoucherDate,a.VoucherSubType,a.AccountID,a.AmountNet,a.DoctorID,a.PatientID,a.OperatorID,a.PatientName,a.PatientAddress1,a.PatientAddress2,a.createdDate,a.CreatedTime, COALESCE(a.CashCounter) from vouchersale a where a.VoucherDate = '" + toDate + "' && a.VoucherType = '" + FixAccounts.VoucherTypeForCashSale + "' && (a.cashCounter is null || a.cashCounter != 'Y') order by  a.VoucherType, a.VoucherNumber";
            dt = DBInterface.SelectDataTable(strSql);
            return dt;
        }
        ...
        public DataTable GetNextVisitDays(string _MFromDate, string _MToDate)
        {
            ...
            try
            {
                string strSql = "select  b.ID  , b.VoucherType, " +
                           ...
                           "where  b.NextVisitDate >= '" + _MFromDate + "'  && b.NextVisitDate <= '" + _MToDate + "' group by AccName order by NextVisitDate, b.PatientName";
                dt = DBInterface.SelectDataTable(strSql);
            }
            catch (Exception Ex)
            {
