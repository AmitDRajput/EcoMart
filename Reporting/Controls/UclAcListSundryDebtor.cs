...
namespace EcoMart.Reporting.Controls
{
    ...
    public partial class UclAcListSundryDebtor : ReportBaseControl
    {
        #region Declaration
        private DataTable _BindingSource;
        private DataTable _MasterAccount;
        ...
        public override void ShowOverview()
        {
            try
            {
                ...
                _MasterAccount = new DataTable();
                _MPReports = new MPReports();
                _FinalAccounts = new FinalAccounts();
                headerLabel1.Text = "ACCOUNT - SUNDRY DEBTOR";
                ClearControls();
                AddToolTip();
                HidepnlGO();
        ...
        private void FillReportData()
        {
            try
            {
                ...
                DataTable _TransactionOP = new DataTable();
                _FinalAccounts.InitializeDBCRFieldsInMasterAccount();
                dtable = _MPReports.GetSundryDebtors(_MToDate);

                _FinalAccounts.UpdatebalancesInMasterAccount(_TransactionOP, dtable);
                ...
                //DataTable dtable = new DataTable();
                //dtable = _MPReports.GetSundryDebtors(_MToDate);
                //_MasterAccount = _MPReports.GetMasterAccount(FixAccounts.AccCodeForDebtor);
                //_BindingSource = dtable;
            }
            catch (Exception ex)
            {
        ...
        private void BindReportGrid()
        {
            try
            {
                ...
                _BindingSource = _FinalAccounts.GetDetailsFromMasterAccount("D");
                foreach (DataRow dr in _BindingSource.Rows)
                {
                    ...
                    if (dr["AccountID"] != DBNull.Value)
                        aaccountID = dr["AccountID"].ToString();
                    if (dr["AccName"] != DBNull.Value)
                        aaccname = dr["AccName"].ToString();
                    if (dr["AccAddress1"] != DBNull.Value)
                        aaccaddress1 = dr["AccAddress1"].ToString();
                    ...
                    if (dr["AccTransactionDebit"] != null)
                        double.TryParse(dr["AccTransactionDebit"].ToString(), out _MDebit);

                    if (dr["AccTransactionCredit"] != null)
                        double.TryParse(dr["AccTransactionCredit"].ToString(), out _MCredit);
        ...
        private void btnOKMultiSelectionClick()
        {
            ...
            try
            {
                ...
                if (retValue)
                {
                    ...
                    ShowpnlGO();
                    FillReportGrid();
                    PrintReportHead = "SUNDRY DEBTORS AS ON " + General.GetDateInShortDateFormat(_MToDate);
                    PrintReportHead2 = "";
