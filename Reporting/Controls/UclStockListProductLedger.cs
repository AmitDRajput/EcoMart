...
namespace EcoMart.Reporting.Controls
{
    ...
    public partial class UclStockListProductLedger : ReportBaseControl
    {
        ...
        public void ClearControls()
        {
            try
            {
                fromDate1.Value = General.ConvertStringToDateyyyyMMdd(General.ShopDetail.Shopsy);
                toDate1.Value = General.TodayDateTime;
        ...
        public void ShowpnlGO()
        {
            ...
            try
            {
                ...
                if (mcbProduct.SeletedItem != null)
        ...
        private void FillReportData()
        {
            try
            {
                ...
                dtable = IsAllowPreviousYearData(dtable);
                _BindingSource = dtable;
                BindReportGrid();
                dtable = _SsStock.GetSaleDataForProductLeder(mcbProduct.SelectedID, _MToDate);
                _BindingSource = dtable;
                BindReportGrid();
                GetSummaryInGrid();
        ...
        private DataTable IsAllowPreviousYearData(DataTable dtable)
        {
            try
            {
                bool IsCheckPreYearData = false;

                AccountingYear _Accyear = new AccountingYear();
                string FromDate = string.Empty;
                DataTable dt = _Accyear.GetOverviewData();

                if (dt != null && dt.Rows.Count > 0)
                {
                    if (dt.Rows.Count == 1)
                    {
                        FromDate = Convert.ToString(dt.Rows[0]["FromDate"]);
                        IsCheckPreYearData = true;
                    }
                    else
                    {
                        IsCheckPreYearData = false;
                    }
                }

                //IsAllowPreYearData = _Accyear.GetYearEndStatus();
                if (IsCheckPreYearData)
                {
                    string filterstr = "VoucherDate <= '" + FromDate + "'";
                    filterstr += "And (VoucherType = '" + FixAccounts.VoucherTypeForCreditNoteStock + "' OR VoucherType = '" + FixAccounts.VoucherTypeForStockIN
                        + "' OR VoucherType = '" + FixAccounts.VoucherTypeForDebitNoteStock + "' OR VoucherType = '" + FixAccounts.VoucherTypeForStockOut + "')";
                    DataRow[] dr = dtable.Select(filterstr);
                    foreach (DataRow drTemp in dr)
                    {
                        dtable.Rows.Remove(drTemp);
                    }
                }
            }
            catch (Exception Ex)
...

        {
            ...
            try
            {
                foreach (DataRow dr in _BindingSource.Rows)
                {
                    ...
                    if (mvoudate == string.Empty || mvoudate == " " || mvoutype == FixAccounts.VoucherTypeForOpeningStock || mvoutype == "Beginning" || Convert.ToInt32(mvoudate) < Convert.ToInt32(_MFromDate))
                        _OpeningStock = _OpeningStock + mqtyin - mqtyout + mscmqtyin - mscmqtyout;

                    else // (mvoudate == string.Empty || mvoudate == " " || mvoutype == FixAccounts.VoucherTypeForOpeningStock || mvoutype == "Beginning" || Convert.ToInt32(mvoudate) >= Convert.ToInt32(_MFromDate))
                    {
                        _RowIndex = dgvReportList.Rows.Add();
                        currentdr = dgvReportList.Rows[_RowIndex];
