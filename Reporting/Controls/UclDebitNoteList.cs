...
namespace EcoMart.Reporting.Controls
{
    ...
    public partial class UclDebitNoteList : ReportBaseControl
    {
        # region Declaration
        ...
        public override void ShowOverview()
        {
            try
            {
                ...
                headerLabel1.Text = "DEBIT NOTE LIST";
            ...
            {               
                ...
                foreach (DataGridViewRow dr in dgvReportList.Rows)
                {
                    ...
                    if (dr.Cells["Col_ID"].Value != null)
                    {
                        ...
                        PrintBill.Rows.Add(row);
                        if (dr.Cells["Col_CVoucherDate"].Value != null && dr.Cells["Col_CVoucherDate"].Value.ToString() != "" && dr.Cells["Col_CVoucherDate"].Value.ToString() != " ")
                            row = new PrintRow(General.GetDateInShortDateFormat(dr.Cells["Col_CVoucherDate"].Value.ToString()), PrintRowPixel, 620, PrintFont);
                        PrintBill.Rows.Add(row);
                        if (dr.Cells["Col_CBillNumber"].Value != null && dr.Cells["Col_CBillNumber"].Value.ToString() != "0")
                            row = new PrintRow(dr.Cells["Col_CBillNumber"].Value.ToString(), PrintRowPixel, 680, PrintFont);
                        PrintBill.Rows.Add(row);
                        if (dr.Cells["Col_Amount"].Value != null)
                        {
                            mamt = Convert.ToDouble(dr.Cells["Col_Amount"].Value.ToString());
                            mlen = (mamt.ToString("#0.00").Length);
                            colpix = Convert.ToInt32(720.00 + ((12.00 - Convert.ToDouble(mlen)) * 5.5));
        ...
        public int PrintHead()
        {
            ...
            try
            {
                ...
                PrintBill.Rows.Add(row);
                row = new PrintRow("BillNo", PrintRowPixel, 690, PrintFont);
                PrintBill.Rows.Add(row);
                    ...
                    if (dr["CRDBID"] != DBNull.Value)
                    {
                        ...
                        int drbillnumber = 0;
                         ...
                         if (dr["VoucherType"] != DBNull.Value)
                             drType = dr["VoucherType"].ToString();
                         if (dr["AmountNet"] != DBNull.Value)
                             drAmt = Convert.ToDouble(dr["AmountNet"].ToString());
                         if (dr["ClearedInVoucherNumber"] != DBNull.Value)
                             drbillnumber = Convert.ToInt32(dr["ClearedInVoucherNumber"].ToString());
                        ifAllow = CheckFilter(drID,drType,drAmt,drbillnumber);
                        if (ifAllow)
                        {                           
                            ...
                            if (dr["ClearedInPurchaseBillNumber"].ToString() != "")
                                dgvrow.Cells["Col_CBillNumber"].Value = dr["ClearedInPurchaseBillNumber"].ToString();
        ...
        private void FillReportData()
        {
            ...
            { 
                ...
                if(cbOnlyPending.Checked == false)
                    dtable = _DebitNoteStock.GetOverviewDataDebitNotes(_MFromDate,_MToDate);
                else
                    dtable = _DebitNoteStock.GetOverviewDataDebitNotesPending(_MFromDate, _MToDate);
        ...
        private void dgvReportList_DoubleClicked(object sender, EventArgs e)
        {
            ...
            try
            {
                if (dgvReportList.SelectedRow != null && dgvReportList.Rows.Count > 0)
                {
                    ...
                    if (mvouchertype == FixAccounts.VoucherTypeForDebitNoteStock)
        ...
        private void dgvReportList_TabKeyPressed(object sender, EventArgs e)
        {
            try
            {
                //TODO: Change code to open second form
                dgvReportList_DoubleClicked(sender, e);
            }
            catch (Exception ex)
            {
                Log.WriteException(ex);
