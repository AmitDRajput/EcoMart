...
namespace PharmaSYSDistributorPlus.Reporting.Controls
{
    ...
    public partial class UclVATListSalesRegister : ReportBaseControl
    {
        ...
        private double _WMTotalZero = 0;
        private double _WMTotalAmount5 = 0;
        private double _WMTotalVAT5 = 0;
        private double _WMTotalAmount12point5 = 0;
        private double _WMTotalVAT12point5 = 0;
        private double _WMTotalRoundoff = 0;
        private double _WMTotalAmount = 0;
        #endregion
        ...
        public override void ShowOverview()
        {
            try
            {
                ...
                {
                    gbDistributor.Visible = true;
                   // pnlMultiSelection1.Size.Height = 211;
                }
                else
                {
                    gbDistributor.Visible = false;
                  //  pnlMultiSelection1.Size.Height = 160;
                }
        ...
        private void FillReportData()
        {
            try
            {
                ...
                //dtable.DefaultView.Sort = "VoucherNumber";
                //DataView TableView = dtable.DefaultView;
        ...
        private void BindReportGrid()
        {
            try
            {
                foreach (DataRow dr in _BindingSource.Rows)
                {
                    ...
                    if ((txtType.Text == "WholeSale" && txtViewtext.Text == "All" && (dr["VoucherType"].ToString() == FixAccounts.VoucherTypeForDistributorSaleCash || dr["VoucherType"].ToString() == FixAccounts.VoucherTypeForDistributorSaleCredit || dr["VoucherType"].ToString() == FixAccounts.VoucherTypeForDistributorSaleCreditStatement)) || (txtType.Text == "Retail" && txtViewtext.Text == "All" && (dr["VoucherType"].ToString() == FixAccounts.VoucherTypeForCashSale || dr["VoucherType"].ToString() == FixAccounts.VoucherTypeForCreditSale || dr["VoucherType"].ToString() == FixAccounts.VoucherTypeForCreditStatementSale)) || (txtType.Text.ToString() == "" || txtType.Text == "Both" || (dr["VoucherType"].ToString() == txtType.Text.ToString())))
                    {
        ...
        private void CheckFilter()
        {
            ...
            try
            {
                ...
                {
                    if (General.ShopDetail.ShopDistributorSale == "Y" && rbtBoth.Checked == true)
                    {
                       //indingSource.DefaultView.RowFilter = " (VoucherType = '" + FixAccounts.VoucherTypeForCashSale + "' or VoucherType = '" + FixAccounts.VoucherTypeForDistributorSaleCash + "' )  and (Voucherdate >= '" + _MFromDate + "' and Voucherdate <= '" + _MToDate + "')";
                        txtViewtext.Text = "Cash";
                        txtType.Text = "Both";
                    }
                    else if (General.ShopDetail.ShopDistributorSale == "Y" && rbtWholesale.Checked == true)
                    {
                      // _BindingSource.DefaultView.RowFilter = " VoucherType = '" + FixAccounts.VoucherTypeForDistributorSaleCash + "' and  Voucherdate >= '" + _MFromDate + "' and voucherdate <= '" + _MToDate + "'";
                        txtViewtext.Text = "Cash";
                        txtType.Text = FixAccounts.VoucherTypeForDistributorSaleCash;
                    }
                    else
                    {
                ...
                else if (rbtnCreditStatement.Checked == true)
                {
                    ...
                    else if (General.ShopDetail.ShopDistributorSale == "Y" && rbtWholesale.Checked == true)
                    {
                        ...
                        txtViewtext.Text = "CreditStatement";
                        txtType.Text = FixAccounts.VoucherTypeForDistributorSaleCreditStatement;
                    }
                    else 
                    {
                        //  _BindingSource.DefaultView.RowFilter = " VoucherType = '" + FixAccounts.VoucherTypeForCreditStatementSale + "'  and   Voucherdate >= '" + _MFromDate + "' and voucherdate <= '" + _MToDate + "'";
                ...
                else if (rbtnCredit.Checked == true)
                {
                    ...
                    else if (General.ShopDetail.ShopDistributorSale == "Y" && rbtWholesale.Checked == true)
                    {
                        //  _BindingSource.DefaultView.RowFilter = " VoucherType = '" + FixAccounts.VoucherTypeForCreditStatementSale + "'  and   Voucherdate >= '" + _MFromDate + "' and voucherdate <= '" + _MToDate + "'";
                        txtViewtext.Text = "Credit";
                        txtType.Text = FixAccounts.VoucherTypeForDistributorSaleCredit;
                    }
                    else
                    {
        ...
        private void CalculateFinalTotals()
        {
            ...
            _WMTotalAmount5 = 0;
            _WMTotalVAT5 = 0;
            _WMTotalAmount12point5 = 0;
            _WMTotalVAT12point5 = 0;
            _WMTotalRoundoff = 0;
            _WMTotalAmount = 0;
            ...
            foreach (DataGridViewRow dr in dgvReportList.Rows)
            {
                ...
                mvoutype = dr.Cells["Col_VoucherType"].Value.ToString();
                if (mvoutype == FixAccounts.VoucherTypeForDistributorSaleCash || mvoutype == FixAccounts.VoucherTypeForDistributorSaleCredit || mvoutype == FixAccounts.VoucherTypeForDistributorSaleCreditStatement)
                {
                    _WMTotalAmount += Convert.ToDouble(dr.Cells["Col_Amount"].Value.ToString());
                    _WMTotalZero += Convert.ToDouble(dr.Cells["Col_AmountZeroPercent"].Value.ToString());
                    _WMTotalAmount5 += Convert.ToDouble(dr.Cells["Col_Amount5Percent"].Value.ToString());
                    _WMTotalVAT5 += Convert.ToDouble(dr.Cells["Col_VAT5Percent"].Value.ToString());
                    _WMTotalAmount12point5 += Convert.ToDouble(dr.Cells["Col_Amount12point5Percent"].Value.ToString());
                    _WMTotalVAT12point5 += Convert.ToDouble(dr.Cells["Col_VAT12Point5Percent"].Value.ToString());
                    _WMTotalRoundoff += Convert.ToDouble(dr.Cells["Col_RoundUpAmount"].Value.ToString());
                }
            ...
            if (General.ShopDetail.ShopDistributorSale == "Y" && (rbtBoth.Checked == true  || rbtWholesale.Checked == true))
            {
                rowIndex = dgvReportList.Rows.Add();
                dgvrow = dgvReportList.Rows[rowIndex];
                dgvrow.DefaultCellStyle.BackColor = General.ReportTotalBackColor;
                dgvrow.DefaultCellStyle.ForeColor = General.ReportTotalForeColor;
                dgvrow.Cells["Col_AccName"].Value = "Total Wholesale";
                dgvrow.Cells["Col_AmountZeroPercent"].Value = _WMTotalZero.ToString("#0.00");
                dgvrow.Cells["Col_Amount5Percent"].Value = _WMTotalAmount5.ToString("#0.00");
                dgvrow.Cells["Col_VAT5Percent"].Value = _WMTotalVAT5.ToString("#0.00");
                dgvrow.Cells["Col_Amount12point5Percent"].Value = _WMTotalAmount12point5.ToString("#0.00");
                dgvrow.Cells["Col_VAT12Point5Percent"].Value = _WMTotalVAT12point5.ToString("#0.00");
                dgvrow.Cells["Col_RoundUpAmount"].Value = _WMTotalRoundoff.ToString("#0.00");
                dgvrow.Cells["Col_Amount"].Value = _WMTotalAmount.ToString("#0.00");
            }
             if (General.ShopDetail.ShopDistributorSale == "Y" && (rbtBoth.Checked == true  || rbtRetail.Checked == true))
             {
                rowIndex = dgvReportList.Rows.Add();
                dgvrow = dgvReportList.Rows[rowIndex];
                dgvrow.DefaultCellStyle.BackColor = General.ReportTotalBackColor;
                dgvrow.DefaultCellStyle.ForeColor = General.ReportTotalForeColor;
                dgvrow.Cells["Col_AccName"].Value = "Total Retail";
                dgvrow.Cells["Col_AmountZeroPercent"].Value = (_MTotalZero - _WMTotalZero).ToString("#0.00");
                dgvrow.Cells["Col_Amount5Percent"].Value = (_MTotalAmount5 -  _WMTotalAmount5).ToString("#0.00");
                dgvrow.Cells["Col_VAT5Percent"].Value = (_MTotalVAT5 -  _WMTotalVAT5).ToString("#0.00");
                dgvrow.Cells["Col_Amount12point5Percent"].Value = (_MTotalAmount12point5 - _WMTotalAmount12point5).ToString("#0.00");
                dgvrow.Cells["Col_VAT12Point5Percent"].Value = (_MTotalVAT12point5 - _WMTotalVAT12point5).ToString("#0.00");
                dgvrow.Cells["Col_RoundUpAmount"].Value = (_MTotalRoundoff - _WMTotalRoundoff).ToString("#0.00");
                dgvrow.Cells["Col_Amount"].Value = (_MTotalAmount -  _WMTotalAmount).ToString("#0.00");
            }
        }
        ...
        #endregion
        public static bool sortingflag = false;
        private void dgvReportList_ColumnHeaderMouseClick(object sender, EventArgs e)
        {
            //if (!sortingflag)
            //{
            //    this.dgvReportList.Sort(this.dgvReportList.Columns["Col_VoucherNumber"], ListSortDirection.Ascending);
            //    sortingflag = true;
            //}
            //else
            //{
            //    this.dgvReportList.Sort(this.dgvReportList.Columns["Col_VoucherNumber"], ListSortDirection.Descending);
            //    sortingflag = false;
            //}
