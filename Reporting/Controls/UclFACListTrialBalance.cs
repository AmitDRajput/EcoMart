    ...
    public partial class UclFACListTrialBalance : ReportBaseControl
    {
      ...
      //  private MPReports _MPReports;
         ...
         public override void Export(string ExportFileName)
        {
            ...
            MReportGridView reportgrid = new MReportGridView();           
            reportgrid = GetReportGrid(reportgrid);
            GeneralReport.ExportFile(PrintReportHead, PrintReportHead2, reportgrid, ExportFileName);
        }

         private MReportGridView GetReportGrid(MReportGridView reportgrid)
         {
             if (dgvReportListLevel4.Visible == true)
                 reportgrid = dgvReportListLevel4;
             else if (dgvReportListLevel3.Visible == true)
                 reportgrid = dgvReportListLevel3;
             else if (dgvReportListLevel2.Visible == true)
                 reportgrid = dgvReportListLevel2;
             else if (dgvReportListLevel1.Visible == true)
                 reportgrid = dgvReportListLevel1;
             else
                 reportgrid = dgvReportList;
             return reportgrid;
         }
        ...
        public override void EMail(string EmailID)
        {
            ...
            MReportGridView reportgrid = new MReportGridView();
            reportgrid = GetReportGrid(reportgrid);
            GeneralReport.SendEmails(PrintReportHead, PrintReportHead2, reportgrid, EmailID);
        }
        ...
        private void FillReportGrid()
        {
            try
            {
                bool retValue =   _FinalAccounts.SaveTrialBalanceDates(_MFromDate,_MToDate);
                FillReportData();
                FormatReportGrid();
                //   FillLevel4();
                FillLevels();
        ...
        private void FillLevels()
        {
            ...
            try
            {
                ...
                FillFinalTotals(_RowIndex, currentdr);
        ...
        private void FillReportData()
        {
            try
            {
                ...
                DataRow rowoftotal  = _FinalAccounts.GetTotalsFromMasterAccount();

                _MTotCLCredit = Convert.ToDouble(rowoftotal["clcr"].ToString());
                _MTotCLDebit = Convert.ToDouble(rowoftotal["cldb"].ToString());
                _MTotOPCredit = Convert.ToDouble(rowoftotal["opcr"].ToString());
                _MTotOPDebit = Convert.ToDouble(rowoftotal["opdb"].ToString());
                _MTotTRCredit = Convert.ToDouble(rowoftotal["trcr"].ToString());
                _MTotTRDebit = Convert.ToDouble(rowoftotal["trdb"].ToString());
        ...
        private void FillFinalTotals(int _RowIndex, DataGridViewRow currentdr)
        {
            //DataGridViewRow currentdr;
            //int _RowIndex = dgvReportList.Rows.Add();
            //currentdr = dgvReportList.Rows[_RowIndex];
            currentdr.DefaultCellStyle.BackColor = General.ReportTotalBackColor;
            currentdr.DefaultCellStyle.ForeColor = General.ReportTotalForeColor;
            currentdr.Cells["Col_AccName"].Value = "Total";
        ...
        private void GetDifferenceRows()
        {
            ...
            try
            {
                dt = _FinalAccounts.GetTrnacTotalsByVoucherTypeNo(_MFromDate,_MToDate);
        ...
        private void dgvReportListLevel4_DoubleClicked(object sender, EventArgs e)
        {
            if (dgvReportListLevel4.SelectedRow != null && dgvReportListLevel4.Rows.Count > 0)
            {
                ...
                if (parno == string.Empty)
                {
                    ...
                    if (acno == FixAccounts.AccountCash.ToString())
