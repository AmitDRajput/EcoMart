...
namespace EcoMart.Reporting.Controls
{
    ...
    public partial class UclAcListGeneralLedger : ReportBaseControl
    {
        #region Declaration
        ...
        public override void ShowOverview()
        {
            try
            {
                ...
                headerLabel1.Text = "ACCOUNT - GENERAL' LEDGER";
                ClearControls();
                FillPartyCombo();
                FillVoucherTypes();
        ...
        private void btnOKMultiSelectionClick()
        {
            ...
            try
            {
                ...
                if (retValue)
                {
                    ...
                    InitializeReportGrid();
                    ShowpnlGO();
                    PrintReportHead = "GENERAL LEDGER From: " + General.GetDateInShortDateFormat(_MFromDate) + " To: " + General.GetDateInShortDateFormat(_MToDate);
                    PrintReportHead2 = "";
                    rowCollection = new List<DataGridViewRow>();
                    if (mcbCreditor.SelectedID != null && mcbCreditor.SelectedID != string.Empty)
                    {
                        ...
                        if (foundintrnac)
                        {
                            ...
                            if (mcbCreditor.SeletedItem.ItemData[5] != null &&  mcbCreditor.SeletedItem.ItemData[5].ToString() != string.Empty)
                                mdebit = Convert.ToDouble(mcbCreditor.SeletedItem.ItemData[5].ToString());
                            if (mcbCreditor.SeletedItem.ItemData[6] != null && mcbCreditor.SeletedItem.ItemData[6].ToString() != string.Empty)
                                mcredit = Convert.ToDouble(mcbCreditor.SeletedItem.ItemData[6].ToString());
                        ...
                        if (rowCollection.Count > 0)
                        {
                            ...
                            foreach (DataGridViewRow row in rowCollection)
                            {
                                ...
                                if (foundintrnac)
                                {
                                    ...
                                    double mopdebit = 0;
                                    double mopcredit = 0;
                                    mid = row.Cells["Col_ID"].Value.ToString();
                                    mname = row.Cells["Col_Name"].Value.ToString();
                                    maddress = row.Cells["Col_Address"].Value.ToString();
                                    mopdebit = Convert.ToDouble(row.Cells["Col_OpeningDebit"].Value.ToString());
                                    mopcredit = Convert.ToDouble(row.Cells["Col_OpeningCredit"].Value.ToString());
                                    BindReportGrid(mid, mname, maddress, mopdebit, mopcredit);
                                }
                            }
                        }
                    }
        ...
        private void FillPartyCombo()
        {
            try
            {
                ...
                DataTable dtable = _Party.GetSSAccountHoldersListForGeneralLedger();
        ...
        private void FillMultiSelectionData()
        {
            try
            {
                ...
                _Account = new Account();
                dtable = _Account.GetSSAccountHoldersListForGeneralLedger();
