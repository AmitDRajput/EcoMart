...
namespace EcoMart.Reporting.Controls
{
    ...
    public partial class UclListTodaysChequesTobeDeposited : ReportBaseControl
    {
        # region Declaration
        ...
        # region Contructor
        public UclListTodaysChequesTobeDeposited()
        {
            try
            {
                InitializeComponent();
            ...
            try
            {
                ...
                foreach (DataGridViewRow dr in dgvReportList.Rows)
                {
                    ...
                    if (dr.Cells["Col_ID"].Value != null)
                    {
                        ...
                        PrintRowPixel += 17;
                        rowcount += 1;
                        row = new PrintRow(srno.ToString(), PrintRowPixel, 1, fnt);
                        PrintBill.Rows.Add(row);
                        row = new PrintRow(dr.Cells["Col_AccName"].Value.ToString().PadRight(30).Substring(0, 30), PrintRowPixel, 30, fnt);
                        PrintBill.Rows.Add(row);
                        row = new PrintRow(dr.Cells["Col_Address"].Value.ToString().PadRight(30).Substring(0, 25), PrintRowPixel, 300, fnt);
                        PrintBill.Rows.Add(row);
                        ...
                        row = new PrintRow(dr.Cells["Col_BankName"].Value.ToString().PadRight(30).Substring(0, 28), PrintRowPixel, 300, fnt);
                        PrintBill.Rows.Add(row);
                        row = new PrintRow(dr.Cells["Col_BranchName"].Value.ToString().PadRight(30).Substring(0, 25), PrintRowPixel, 500, fnt);
                        PrintBill.Rows.Add(row);
        ...
        private int PrintHeader(int TotalPages, int Rowcount, Font fnt)
        {
            ...
            try
            {
                string reportHead = "Cheques ToBe Depodited Bank :"+ ViewText.Text.ToString()+"  From : " + General.GetDateInDateFormat(_MFromDate) + " To : " + General.GetDateInDateFormat(_MToDate);
                ...
                PrintBill.Rows.Add(row);              
                row = new PrintRow("Party", PrintRowPixel, 30, fnt);
                PrintBill.Rows.Add(row);
                row = new PrintRow("Address", PrintRowPixel, 300, fnt);
                PrintBill.Rows.Add(row);
        ...
        public void ShowpnlGO()
        {
            ...
            ViewText.Text = mcbBank.SeletedItem.ItemData[2];
        ...
        private void ConstructReportColumns()
        {
            try
            {
                ...
                column = new DataGridViewTextBoxColumn();
                column.Name = "Col_BranchName";
                column.DataPropertyName = "BranchName";
                column.HeaderText = "Branch";
                column.Width = 150;
                column.AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells;
                dgvReportList.Columns.Add(column);
        ...
        private void FillBankCombo()
        {
            try
            {
                ...
                mcbBank.SourceDataString = new string[5] { "AccountID", "AccCode", "AccName", "AccAddress1","AccBankAccountNumber" };
                mcbBank.ColumnWidth = new string[5] { "0", "20", "200", "0","200" };
        ...
        private void FillReportData()
        {
            ...
            {
                _BindingSource = new DataTable();
                _BindingSource = _BankPayment.GetOverviewDataForTodaysChequesTobeDeposited(_VouType, _MFromDate, _MToDate, mcbBank.SelectedID);
            }
            catch (Exception ex)
            {
                Log.WriteException(ex);
        ...
        private void btnOKMultiSelectionClick()
        {
            try
            {
                ...
                FillReportGrid();
                ShowpnlGO();
                PrintReportHead = "Cheques ToBe Depodited Bank :" + ViewText.Text.ToString() + "  From : " + General.GetDateInDateFormat(_MFromDate) + " To : " + General.GetDateInDateFormat(_MToDate);
                PrintReportHead2 = "";
