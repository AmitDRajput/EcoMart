...
{
    public partial class UclChallanPurchaseList : ReportBaseControl
    {
        #region Declaration
            ...
            try
            {
                ...
                foreach (DataGridViewRow dr in dgvReportList.Rows)
                {
                    ...
                    if (dr.Cells["Col_ID"].Value != null)
                    {
                        ...
                        {
                            row = new PrintRow(dr.Cells["Col_ChallanNumber"].Value.ToString(), PrintRowPixel, 15, PrintFont);
                            PrintBill.Rows.Add(row);
                        }
                        if (string.IsNullOrEmpty(Convert.ToString(dr.Cells["Col_ChallanDate"].Value)) == false)
                        {
                            row = new PrintRow(General.GetDateInShortDateFormat(dr.Cells["Col_ChallanDate"].Value.ToString()), PrintRowPixel, 100, PrintFont);
                            PrintBill.Rows.Add(row);
                        }
                        if (string.IsNullOrEmpty(Convert.ToString(dr.Cells["Col_AccName"].Value)) == false)
                        {
                            ...
                            row = new PrintRow((dr.Cells["Col_AccName"].Value.ToString()).Substring(0, length), PrintRowPixel, 180, PrintFont);
                            PrintBill.Rows.Add(row);
                        }
                        ...
                        {
                            row = new PrintRow(dr.Cells["Col_VoucherNumber"].Value.ToString(), PrintRowPixel, 570, PrintFont);
                            PrintBill.Rows.Add(row);
                        }
                        if (string.IsNullOrEmpty(Convert.ToString(dr.Cells["Col_PurchaseBillNumber"].Value)) == false)
                        {
                            row = new PrintRow(dr.Cells["Col_PurchaseBillNumber"].Value.ToString(), PrintRowPixel, 650, PrintFont);
                            PrintBill.Rows.Add(row);
                        }
                        if (string.IsNullOrEmpty(Convert.ToString(dr.Cells["Col_VoucherDate"].Value)) == false)
                        {
                            row = new PrintRow(General.GetDateInShortDateFormat(dr.Cells["Col_VoucherDate"].Value.ToString()), PrintRowPixel, 730, PrintFont);
                            PrintBill.Rows.Add(row);
                        }
                    }
                }
        ...
        public int PrintHead()
        {
            ...
            try
            {
                ...
                row = new PrintRow("ChallanNumber", PrintRowPixel, 1, PrintFont);
                PrintBill.Rows.Add(row);

                row = new PrintRow("ChallanDate", PrintRowPixel, 100, PrintFont);
                PrintBill.Rows.Add(row);
                row = new PrintRow("Party", PrintRowPixel, 180, PrintFont);
                PrintBill.Rows.Add(row);
                row = new PrintRow("Amount", PrintRowPixel, 490, PrintFont);
                PrintBill.Rows.Add(row);
                row = new PrintRow("VoucherNumber", PrintRowPixel, 550, PrintFont);
                PrintBill.Rows.Add(row);
                row = new PrintRow("Pur BillNumber", PrintRowPixel, 640, PrintFont);
                PrintBill.Rows.Add(row);
                row = new PrintRow("Bill Date", PrintRowPixel, 730, PrintFont);
                PrintBill.Rows.Add(row);
        ...
        private void ConstructReportColumns()
        {
            try
            {
                ...
                column.Visible = false;
                column.Width = 115; column.AutoSizeMode = DataGridViewAutoSizeColumnMode.ColumnHeader;
                dgvReportList.Columns.Add(column);
                ...
                column.HeaderText = "Purchase VoucherNumber";
                column.Width = 70;
                column.AutoSizeMode = DataGridViewAutoSizeColumnMode.ColumnHeader;
                dgvReportList.Columns.Add(column);
                ...
                column.HeaderText = "Purchase BillNumber";
                column.Width = 75;
                column.AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells;
                dgvReportList.Columns.Add(column);
            ...
            try
            {
                ...
                foreach (DataRow dr in _BindingSource.Rows)
                {
                    if (dr["ID"] != DBNull.Value)
                    {
                        ...
                        if (dr["VoucherDate"].ToString() != "")
                            dgvrow.Cells["Col_VoucherDate"].Value = dr["VoucherDate"].ToString();
                        if (dr["PurchaseBillNumber"].ToString() != "")
            ...
            try
            {
                ...
                if (mcbParty.SeletedItem == null)
                {
                    if (rbtnPending.Checked == true)
                        dtable = _ChallanPurchase.GetOverviewDataForPendingChallanSummary(_MFromDate, _MToDate);
                    else
                        dtable = _ChallanPurchase.GetOverviewDataForAllChallanSummary(_MFromDate, _MToDate);
                }
                else
                {
                    string PartyID = mcbParty.SelectedID;
                    if (rbtnPending.Checked == true)
                        dtable = _ChallanPurchase.GetOverviewDataForPartyPendingChallanSummary(PartyID, _MFromDate, _MToDate);
                    else
                        dtable = _ChallanPurchase.GetOverviewDataForPartyAllChallanSummary(PartyID, _MFromDate, _MToDate);
                }
