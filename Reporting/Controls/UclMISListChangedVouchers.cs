...
namespace EcoMart.Reporting.Controls
{
    ...
    public partial class UclMISListChangedVouchers : ReportBaseControl
    {
        #region Declaration
        ...
        # region Constructor
        public UclMISListChangedVouchers()
        {
            try
            {
                InitializeComponent();
        ...
        public override void ShowOverview()
        {
            try
            {
                ...
                headerLabel1.Text = "CHANGED VOUCHERS";
                ClearControls();
                AddToolTip();
                HidepnlGO();
            }
        ...
        private void ConstructReportColumns()
        {
            try
            {
                ...
                column.Name = "Col_ID";
                column.DataPropertyName = "ChangedID";
        ...
        private void FillReportData()
        {
            try
            {
                ...
                _DataSourceStockOut = new DataTable();        
                if (txtType.Text != "")
                    dtable = _DeletedORChangedVouchers.GetChangedDataMasterSale(_MFromDate, _MToDate);
                else
                {
                    dtable = _DeletedORChangedVouchers.GetChangedDataMasterSale(_MFromDate, _MToDate);
                }
                _DataSourcePurchase = _DeletedORChangedVouchers.GetChangedDataMasterPurchase(_MFromDate, _MToDate);
                _DataSourceCashReceipt = _DeletedORChangedVouchers.GetChangedDataMasterCashReceipt(_MFromDate, _MToDate);
                _DataSourceBankReceipt = _DeletedORChangedVouchers.GetChangedDataMasterBankReceipt(_MFromDate, _MToDate);
                _DataSourceCashPayment = _DeletedORChangedVouchers.GetChangedDataMasterCashPayment(_MFromDate, _MToDate);
                _DataSourceBankPayment = _DeletedORChangedVouchers.GetChangedDataMasterBankPayment(_MFromDate, _MToDate);
            ...
            try
            {
                foreach (DataRow dr in _BindingSource.Rows)
                {
                    if (txtType.Text.ToString() == "" || (dr["VoucherType"].ToString() == txtType.Text.ToString()))
                    {
                        ...
                        dgvrow.Cells["Col_ID"].Value = dr["ChangedID"].ToString();
