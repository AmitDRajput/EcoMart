...
namespace EcoMart.Reporting.Controls
{
    ...
    public partial class UclAcListBankBookByClearedDate : ReportBaseControl
    {
        ...
        # region Constructor
        public UclAcListBankBookByClearedDate()
        {
            try
            {
                InitializeComponent();
        ...
        public override void ShowOverview()
        {
            try
            {
                ...
                headerLabel1.Text = "ACCOUNT - BANK BOOK BY CLEARED DATE";
                ClearControls();
                FillVoucherTypes();
                FillBankCombo();
                AddToolTip();
        ...
        public void ShowpnlGO()
        {
            ...
            if (General.CurrentSetting.MsetEmailID != null && General.CurrentSetting.MsetEmailID.ToString().Trim() != string.Empty)
            {
                tsBtnEMail.Enabled = true;
            }
            ViewFromDate.Text = General.GetDateInShortDateFormat(_MFromDate);
            ViewToDate.Text = General.GetDateInShortDateFormat(_MToDate);
            if (mcbBank.SeletedItem.ItemData[2] != null)
            txtViewtext.Text = mcbBank.SeletedItem.ItemData[2];
            if (mcbBank.SeletedItem.ItemData[3] != null)
                txtViewtext.Text = mcbBank.SeletedItem.ItemData[2].Trim() + " " + mcbBank.SeletedItem.ItemData[3].Trim();
            dgvReportList.Focus();
        }
        ...
        private void FillReportData()
        {
            try
            {
                ...
                dtable = _MPReports.GetGeneralLedgerByClearedDate(_MFromDate, _MToDate, mcbBank.SelectedID);
        ...
        private void BindReportGrid(string AccID, string party, string narr, double opDebit, double opCredit)
        {
            try
            {
                ...
                {
                    if (dr["AccountID"].ToString().Trim() == AccID && Convert.ToInt32(dr["ClearedDate"].ToString()) >= Convert.ToInt32(_MFromDate) && Convert.ToInt32(dr["ClearedDate"].ToString()) <= Convert.ToInt32(_MToDate))
                    {
                        bool ifadd = false;
                        ...
                        if (ifadd)
                        {
                            ...
                            currentdr.Cells["Col_VoucherDate"].Value = dr["ClearedDate"].ToString();
        ...
        private void btnOKMultiSelectionClick()
        {
            ...
            try
            {
                ...
                if (retValue)
                {
                    ...
                    InitializeReportGrid();
                    ShowpnlGO();
                    PrintReportHead = "Bank Book By Cleared Date From: " + General.GetDateInShortDateFormat(_MFromDate) + " To: " + General.GetDateInShortDateFormat(_MToDate);
