...
namespace EcoMart.Reporting.Controls
{
    ...
    public partial class UclStockListCurrentStock : ReportBaseControl
    {
        # region Declaration
        ...
        // for show report initialize fromdate and todate
        string _MFromDate = General.ShopDetail.Shopsy;
        string _MToDate = DateTime.Now.Date.ToString("yyyyMMdd");
        # endregion

        # region Constructor
        public UclStockListCurrentStock()
        {
            try
            {
                InitializeComponent();
        ...
        public override void ShowOverview()
        {
            try
            {
                ...
                headerLabel1.Text = "STOCK-CURRENT STOCK PRODUCTWISE";
        ...
        private int PrintHead()
        {
            ...
            try
            {
                ...
                {
                    row = new PrintRow("Cl Stock", PrintRowPixel, 420, PrintFont);
                    PrintBill.Rows.Add(row);
                }
                else
                {
                    row = new PrintRow("OP Stock", PrintRowPixel, 420, PrintFont);
                    PrintBill.Rows.Add(row);
                }
                PrintRowPixel += 13;
        ...
        private void BindReportGrid(string ID, string party, string narr, double opDebit, double opCredit)
        {
            try
            {
                int _RowIndex;
        ...
        private void GetStockDataProductwise(string compID)
        {
            DataTable dtable = new DataTable();           
             if (cbZero.Checked == true)
                 dtable = _SsStock.GetStockForAll(compID);
             else
             {
                 if (rbtOpeningSTock.Checked == true)
                     dtable = _SsStock.GetStockForAllWithOutZeroOpening(compID);
                 else
                     dtable = _SsStock.GetStockForAllWithOutZeroClosing(compID);
             }
             _BindingSource = dtable;
        }
        ...
        private void FillReportGridforoneCompany()
        {
        ...
        //    try
        //    {
        //        if (btnOKMultiSelection1.Visible != true || (btnOKMultiSelection1.Visible == true && cbSelectAll.Visible == false))
        //        {
        //            foreach (DataGridViewRow drow in dgvMultiSelection.Rows)
        //            {
