    ...
    public partial class UclDebitNoteAmount : BaseControl
    {
        ...
        public override bool FillSearchData(string ID, string Vmode)
        {
            if (ID != null && ID != "")
            {
                ...
                {
                    lblMessage.Text = "Debit Note Cleared in Purchase Voucher:" + _DNAmount.ClearVouType + " " + _DNAmount.ClearVouNo.ToString();
                    Cancel();
                }
                else
                {
        ...
        private void txtVouchernumber_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                if (txtVouchernumber.Text != null)
                {
                    ...
                    _DNAmount.ReadDetailsByVouNumber(vouno);
        ...
        {
            //ttDBNAmount.SetToolTip(txtBillAmount, "NetAmount+Vat5%Amount+Vat12.5%Amount-Discount");
            //ttDBNAmount.SetToolTip(txtNarration, "Enter Narration");
            //ttDBNAmount.SetToolTip(lblNetAmount, "Amount+VatInput5%+VatInput12.5%-Discount");
            //ttDBNAmount.SetToolTip(txtDiscAmount, "(Amount+VatInput5%+VatInput12.5%)*Discount%");
            //ttDBNAmount.SetToolTip(txtDiscPercent, "Enter Discount percent");
            //ttDBNAmount.SetToolTip(txtVatInput12point5per, "Enter VAT Amount 12.5%");
            //ttDBNAmount.SetToolTip(txtVatInput5per, "Enter VAT Amount 5%");
            //mcbCreditor.SetToolTip(ttDBNAmount, "Fill Creditor");
            //mcbCreditor.SetToolTipNewButton(ttDBNAmount, "Click to create new Creditor");
        }
