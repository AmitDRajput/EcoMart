    ...
    public partial class UclCashExpenses : BaseControl
    {
        ...
        private void InitialisempPVC1()
        {
            try
            {
                ...
                mpMainSubViewControl1.NumericColumnNames.Add("Col_Debit");
                mpMainSubViewControl1.NumericColumnNames.Add("Col_Credit");
            ...
            try
            {
                foreach (DataGridViewRow dr in mpMainSubViewControl1.Rows)
                {
                    ...
                    if (dr.Cells["Col_ID"].Value != null && dr.Cells["Col_ID"].Value.ToString() != "")
                    {
                        ...
                        if (dr.Cells["Col_Credit"].Value != null)
                            double.TryParse(dr.Cells["Col_Credit"].Value.ToString(), out mcredit);
                        retValue = _CashExpenses.AddParticularsDetails(_CashExpenses.Id, macno, mdebit, mcredit, _CashExpenses.DetailId);
                        if (retValue == false)
                            break;
                    }
                }
            ...
            try
            {
                foreach (DataGridViewRow dr in mpMainSubViewControl1.Rows)
                {
                    ...
                    if (dr.Cells["Col_ID"].Value != null && dr.Cells["Col_ID"].Value.ToString() != "")
                    {
                        ...
                        retValue = _CashExpenses.AddDetailsInmaterJV(_CashExpenses.JVID, macno, FixAccounts.VoucherTypeForJournalEntry, _CashExpenses.JVNo, General.ShopDetail.ShopVoucherSeries, _CashExpenses.CBVouDate, mdebit, mcredit, _CashExpenses.Id, _CashExpenses.CBNarration, _CashExpenses.CreatedBy, _CashExpenses.CreatedDate, _CashExpenses.CreatedTime);
                        _CashExpenses.DetailId = Guid.NewGuid().ToString().ToUpper().Replace("-", "");
                        retValue = _CashExpenses.AddJVIntblTrnac(_CashExpenses.JVID, macno, mdebit, mcredit, _CashExpenses.DetailId, _CashExpenses.CBAccountID, _CashExpenses.CBVouDate, FixAccounts.VoucherTypeForJournalEntry, _CashExpenses.Id, _CashExpenses.CBNarration, _CashExpenses.JVNo, _CashExpenses.CreatedBy, _CashExpenses.CreatedDate, _CashExpenses.CreatedTime);
                        _CashExpenses.DetailId = Guid.NewGuid().ToString().ToUpper().Replace("-", "");
                        retValue = _CashExpenses.AddJVIntblTrnacReverse(_CashExpenses.JVID, macno, mdebit, mcredit, _CashExpenses.DetailId, _CashExpenses.CBAccountID, _CashExpenses.CBVouDate, FixAccounts.VoucherTypeForJournalEntry, _CashExpenses.Id, _CashExpenses.CBNarration, _CashExpenses.JVNo, _CashExpenses.CreatedBy, _CashExpenses.CreatedDate, _CashExpenses.CreatedTime);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void txtVouchernumber_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    {
                        _CashExpenses.CBVouNo = Convert.ToInt32(txtVouchernumber.Text.ToString());
                        _CashExpenses.ReadDetailsByVouNumber();
                        FillSearchData(_CashExpenses.Id,"");
                    }
                }
            }
            catch (Exception Ex)
