...
namespace PharmaSYSRetailPlus.DataLayer
{
    public class DBCorrection
    {
        ...
        {
            DataTable dtable = new DataTable();
            string strSql = "Select a.ID,a.VoucherType,a.VoucherNumber,a.VoucherDate,a.ProductID, b.ProdName, b.ProdLoosePack, b.ProdPack, b.ProdCompShortName  from vouchercorrectioninrate a inner join masterproduct b on a.Productid = b.ProductId order by a.VoucherNumber";
            dtable = DBInterface.SelectDataTable(strSql);
            return dtable;
        }

        public bool AddDetails(string stockid, string productId , string BatchNumber, string Expiry,
        double MRP, double PurchaseRate, double SellRate, int Qty, double traderate, string expirydate, double productvat, double purchasevat, string lastpurchasedate, string lastpurchaseaccountid)
            {
            bool bRetValue = false;
            string strSql = GetInsertQuery(stockid,productId, BatchNumber, Expiry, MRP, PurchaseRate, SellRate, Qty,traderate,expirydate,productvat,purchasevat,lastpurchasedate,lastpurchaseaccountid) ;
        ...
        double MRP, double PurchaseRate, double SaleRate, int Qty, double traderate, string expirydate, double productvat, double purchasevat, string lastpurchasedate, string lastpurchaseaccountid)
        {
        ...
        public bool AddDetailsInVoucherCorrection(string ID, string Oldstockid, string NewStockID, int Vouchernumber, string Voucherdate, string productId, string OldBatch, string OldExpiry,
                    ...
                    string NewBatch, string NewExpiry, double NewMRP, double NewPurchaseRate, double NewSaleRate, int NewQty, string createdby, string createddate, string createdtime)
        {
            bool bRetValue = false;
            string strSql = GetInsertQueryInVoucherCorrection(ID, Oldstockid, NewStockID, Vouchernumber, Voucherdate, productId, OldBatch, OldExpiry,
                               OldMRP, OldPurchaseRate, OldSellRate, OldQty, NewBatch, NewExpiry, NewMRP, NewPurchaseRate, NewSaleRate, NewQty,createdby,createddate,createdtime);
        ...
        public DataRow GetStockByStockID(string stockID)
        {
            ...
            string strSql = string.Format("Select StockID,ProductId,BatchNumber,MRP,Expiry,ExpiryDate,TradeRate,PurchaseRate,SaleRate,OpeningStock,ClosingStock,PurchaseStock ,TransferInStock,CreditNoteStock,SaleStock,TransferOutStock,DebitNoteStock,PurchaseSchemeStock,PurchaseReplacementStock,SaleSchemeStock,IfRateCorrection,ProductVATPercent,PurchaseVATPercent,LastPurchaseAccountId,LastPurchaseBillNumber,LastPurchaseDate,LastPurchaseVoucherType,LastPurchaseVoucherNumber,ScanCode,CreatedDate,CreatedUserId,ModifiedDate,ModifiedUserId from tblstock where StockID = '{0}'", stockID);
            dataRow = DBInterface.SelectFirstRow(strSql);
            return dataRow;
        }
        public bool UpdateDetails(string StockID, string Expiry,string expirydate, double PurchaseRate, double SaleRate)
        {
            bool bRetValue = false;
            string strSql = GetUpdateQuery(StockID , Expiry, expirydate, PurchaseRate, SaleRate);
            if (DBInterface.ExecuteQuery(strSql) > 0)
            {
                bRetValue = true;
            }
        ...
        private string GetUpdateQuery(string stockid, string Expiry, string ExpiryDate,double PurchaseRate,double SellRate)
        {
            Query objQuery = new Query();
            objQuery.Table = "tblstock";
