...
namespace PharmaSYSRetailPlus.DataLayer
{
    class DBUser
    {
        ...
        public bool AddDetails(string ID, string username, string mpassword,int IfInUse, int mlevel, string details, string createdby, string createddate, string createdtime)
        {
            bool bRetValue = false;
            string strSql = GetInsertQuery(ID, username, mpassword, IfInUse, mlevel, details, createdby, createddate,createdtime);
        ...
        private string GetInsertQuery(string ID, string username, string mpassword, int IfInUse, int mlevel, string details ,string createdby,string createddate, string createdtime)
        {
        ...
        public DataTable GetOverviewData()
        {
            ...
            DataTable dtable = new DataTable();
            string strSql = string.Format("Select UserID, UserName, Password, Level,IfInUse,UserDetails from tbluser  order by UserName");
            dtable = DBInterface.SelectDataTable(strSql);
            return dtable;
        }
        ...
        {
            DataTable dtable = new DataTable();
            string strSql = "select ID,Type from tbluserlevel order by ID";
            dtable = DBInterface.SelectDataTable(strSql);
            return dtable;
        }
        public bool UpdateDetails(string id, string UserName, string Password, int Level,int ifinuse, string Details,string modifiedby,string modifieddate,string modifiedtime)
        {
            bool bRetValue = false;
            string strSql = GetUpdateQuery(id,UserName, Password, Level,ifinuse, Details,modifiedby,modifieddate,modifiedtime);
        ...
        private string GetUpdateQuery(string id, string UserName, string Password, int Level, int ifinuse, string Details, string modifiedby, string modifieddate, string modifiedtime)
        {
            Query objQuery = new Query();
            objQuery.Table = "tbluser";
        ...
        public DataRow ReadDetailsByID(string Id)
        {
            ...
            if (Id != "")
            {
                string strSql = "Select a.UserID,a.UserName,a.Password,a.Level,a.IfInUse,a.UserDetails,b.Type from tbluser a inner join tbluserlevel b on a.level = b.ID  where UserID='{0}' ";
