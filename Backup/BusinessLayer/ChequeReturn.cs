    ...
    class ChequeReturn : BaseObject
    {
        ...
        public bool AddAccountDetailsIntbltrnacDebitForChequeReturn()
        {
            ...
            try
            {
                ...
                if (CBAmount > 0)
                {
                    bRetValue = dbpur.AddDetailsForAccountsBankReceipt(Id, DetailId, CBAccountID, CBAmount, 0, CBBankAccountID,CBVouType , CBVouNo, CBVouDate, CBNarration, CBBankID, CBBranchID, CBChequeNumber, CBChequeDate, CreatedBy, CreatedDate, CreatedTime);
                }
            }
            catch (Exception ex)
            {
