...
{
    public class ChallanPurchase : BaseObject
    {
        #region Declaration
        ...
        private double _preAmountLessS;
        ...
        public double preAmountLessS
        {
            get { return _preAmountLessS; }
            set { _preAmountLessS = value; }
        }
        ...
        public override void DoValidate()
        {
            ...
            try
            {
                ...
                if (ChallanNumber == "")
                    ValidationMessages.Add("Please enter the  ChallanNumber");
                if (AmountNetS == 0)
                    ValidationMessages.Add("Can Not Save Zero Amount Purchase");
                //if (TransactionText == "")
                //    ValidationMessages.Add("Select Transaction Type");
                if (VoucherType.Trim() == string.Empty)
                    ValidationMessages.Add("Select Transaction Type");
                if (Id == "")
                    retValue = CheckForUniqueBillNumberforNew(ChallanNumber, AccountID);
                else
                    retValue = CheckForUniqueBillNumberforEdit(Id, ChallanNumber, AccountID);
                if (retValue == false)
                {
                    ValidationMessages.Add("Challan Purchase Number Already Entered");

                }

                retValue = General.CheckDates(VoucherDate, VoucherDate);
                if (retValue == false)
                {
                    ValidationMessages.Add("Please Check Voucher Date");
                }
        ...
        public bool ReadDetailsByID()
        {
            ...
            {
                DataRow drow = null;
                DBChallanPurchase dbData = new DBChallanPurchase();
                drow = dbData.ReadDetailsByID(Id);
                if (drow != null)
                {
                    if (drow["ID"] != DBNull.Value)
                        Id = Convert.ToString(drow["ID"]);
                    if (drow["EntryDate"] != DBNull.Value)
                        EntryDate = Convert.ToString(drow["EntryDate"]);
                    if (drow["ChallanNumber"] != DBNull.Value)
                        ChallanNumber = Convert.ToString(drow["ChallanNumber"]);
                    ...
                    if (drow["VoucherSubType"] != DBNull.Value)
                        VoucherSubType = drow["VoucherSubType"].ToString();
        ...
        public DataTable ReadProductDetailsByID()
        {
            ...
            try
            {
                DBChallanPurchase dbp = new DBChallanPurchase();
        ...
        public DataTable ReadProductDetailWithoutPurchaseByID()
        {
            DataTable dt = new DataTable();
            dt = null;
            try
            {
                DBChallanPurchase dbp = new DBChallanPurchase();
                dt = dbp.ReadProductDetailWithoutPurchaseByID(Id);
            }
            catch (Exception Ex)
            {
                Log.WriteException(Ex);
        ...
        public DataTable ReadProductDetailsByIDPurchaseAndDetailID()
        {
            DataTable dt = new DataTable();
            dt = null;
            try
            {
                DBChallanPurchase dbp = new DBChallanPurchase();
                dt = dbp.ReadProductDetailsByIDPurchaseAndDetailID(Id, DetailId);
            }
            catch (Exception Ex)
            {
                Log.WriteException(Ex);
        ...
        public int GetAndUpdateChallanPurchaseNumber(string vtype)
        {
            int vouno = 0;
            try
            {
                ...
                vouno = dbno.GetChallanPurchase(vtype, General.ShopDetail.ShopVoucherSeries);
            }
            catch (Exception Ex)
            {
                Log.WriteException(Ex);
        ...
        {
            DBChallanPurchase dbcrdb = new DBChallanPurchase();
            return dbcrdb.AddDetails(Id, AccountID, Narration, EntryDate, ChallanNumber, VoucherSeries, VoucherType, VoucherNumber, VoucherDate,
           ...
           AmountAddOnFreightS, AmountLessS, CashDiscountPercentageS, AmountCreditNoteS, AmountDebitNoteS, RoundUpAmountS,
        ...
        //   preAmountAddOnFreightS, preAmountLessS, preCashDiscountPercentageS, preAmountCreditNoteS, preAmountDebitNoteS, preRoundUpAmountS,
        ...
        {
            DBChallanPurchase dbpur = new DBChallanPurchase();
            return dbpur.AddDetailsProductsSS(Id, ChallanNumber, VoucherDate, VoucherNumber, ProductID, Batchno, TradeRate, PurchaseRate, MRP, SaleRate,
                ...
                IfTradeRateInclusiveOfVAT, Amount, AmountSplDiscountPerUnit, SplDiscountPercent, AmountZeroVAT, AmountCashDiscountPerUnit, StockID, DetailId, ProductMargin, ProductMargin2, SerialNumber, PurScanCode, DistributorSaleRatePercent, DistributorSaleRate);
        }
        ...
        //            DebitAccount = FixAccounts.AccountCashPurchase;
        //            PurchaseAccount = FixAccounts.AccountCashPurchase;
        //        }
        //        else if (VoucherType == FixAccounts.VoucherTypeForCreditPurchase)
        //        {
        //            CreditAccount = AccountID;
        //            DebitAccount = FixAccounts.AccountCashCreditPurchase;
        //            PurchaseAccount = FixAccounts.AccountCashCreditPurchase;
        //        }
        //        else
        //        {
        //            CreditAccount = AccountID;
        //            DebitAccount = FixAccounts.AccountCreditPurchase;
        //            PurchaseAccount = FixAccounts.AccountCreditPurchase;
        //        }
        ...
        //            bRetValue = dbpur.AddDetailsForAccountsPurchase(Id, DetailId, DebitAccount, PurchaseAmountZeroVATS, 0, CreditAccount, VoucherType, VoucherNumber, VoucherDate, Narration, VoucherSubType, CreatedBy, CreatedDate, CreatedTime);
        //        }
        //        ////if (bRetValue == true && (AmountBillS - PurchaseAmountZeroVATS) > 0)
        //        ////{
        //        ////    DetailId = Guid.NewGuid().ToString().ToUpper().Replace("-", "");
        //        ////  // DebitAccount = FixAccounts.
        //        ////    //if (VoucherSubType == "2")
        //        ////    //{
        //        ////    //    bRetValue = dbpur.AddDetailsForAccountsPurchase(Id, DetailId, DebitAccount, AmountBillS+AmountSchemeDiscountS - PurchaseAmountZeroVATS, 0, CreditAccount, VoucherType, VoucherNumber, VoucherDate, Narration, VoucherSubType, CreatedBy, CreatedDate, CreatedTime);
        //        ////    //}
        //        ////    //else
        //        ////    //{
        //        ////        bRetValue = dbpur.AddDetailsForAccountsPurchase(Id, DetailId, DebitAccount, AmountBillS - PurchaseAmountZeroVATS, 0, CreditAccount, VoucherType, VoucherNumber, VoucherDate, Narration, VoucherSubType, CreatedBy, CreatedDate, CreatedTime);
        //        ////    //}
        //        ////}
        //        if (bRetValue == true && PurchaseAmount5PercentVATS > 0)
        //        {
        //            DetailId = Guid.NewGuid().ToString().ToUpper().Replace("-", "");
        //            DebitAccount = FixAccounts.AccountVAT6Purchase;
        //            bRetValue = dbpur.AddDetailsForAccountsPurchase(Id, DetailId, DebitAccount, PurchaseAmount5PercentVATS, 0, CreditAccount, VoucherType, VoucherNumber, VoucherDate, Narration, VoucherSubType, CreatedBy, CreatedDate, CreatedTime);
        //        }
        //        if (bRetValue == true && PurchaseAmount12point5PercentVATS > 0)
        //        {
        //            DetailId = Guid.NewGuid().ToString().ToUpper().Replace("-", "");
        //            DebitAccount = FixAccounts.AccountVAT13point5Purchase;
        //            bRetValue = dbpur.AddDetailsForAccountsPurchase(Id, DetailId, DebitAccount, PurchaseAmount12point5PercentVATS, 0, CreditAccount, VoucherType, VoucherNumber, VoucherDate, Narration, VoucherSubType, CreatedBy, CreatedDate, CreatedTime);
        //        }
        //        if (bRetValue == true && AmountVAT5PercentS > 0)
        //        {
        //            DetailId = Guid.NewGuid().ToString().ToUpper().Replace("-", "");
        //            DebitAccount = FixAccounts.AccountVatInput6Purchase;
        //            bRetValue = dbpur.AddDetailsForAccountsPurchase(Id, DetailId, DebitAccount, AmountVAT5PercentS, 0, CreditAccount, VoucherType, VoucherNumber, VoucherDate, Narration, VoucherSubType, CreatedBy, CreatedDate, CreatedTime);
        //        }
        //        if (bRetValue == true && AmountVAT12point5PercentS > 0)
        //        {
        //            DetailId = Guid.NewGuid().ToString().ToUpper().Replace("-", "");
        //            DebitAccount = FixAccounts.AccountVatInput13point5Purchase;
        //            bRetValue = dbpur.AddDetailsForAccountsPurchase(Id, DetailId, DebitAccount, AmountVAT12point5PercentS, 0, CreditAccount, VoucherType, VoucherNumber, VoucherDate, Narration, VoucherSubType, CreatedBy, CreatedDate, CreatedTime);
        //        }
        //if (bRetValue == true && AmountItemDiscountS > 0)
        //{
        //    DetailId = Guid.NewGuid().ToString().ToUpper().Replace("-", "");
        //    DebitAccount = FixAccounts.AccountItemDiscountPurchase;
        //    bRetValue = dbpur.AddDetailsForAccountsPurchase(Id, DetailId, DebitAccount, 0, AmountItemDiscountS, CreditAccount, VoucherType, VoucherNumber, VoucherDate, Narration, VoucherSubType, CreatedBy, CreatedDate, CreatedTime);
        //}
        //if (bRetValue == true && AmountSpecialDiscountS > 0)
        //{
        //    DetailId = Guid.NewGuid().ToString().ToUpper().Replace("-", "");
        //    DebitAccount = FixAccounts.AccountSplDiscountPurchase;
        //    bRetValue = dbpur.AddDetailsForAccountsPurchase(Id, DetailId, DebitAccount, 0, AmountSpecialDiscountS, CreditAccount, VoucherType, VoucherNumber, VoucherDate, Narration, VoucherSubType, CreatedBy, CreatedDate, CreatedTime);
        //}
        //if (bRetValue == true && AmountCashDiscountS > 0)
        //{
        //    DetailId = Guid.NewGuid().ToString().ToUpper().Replace("-", "");
        //    DebitAccount = FixAccounts.AccountCashDiscountPurchase;
        //    bRetValue = dbpur.AddDetailsForAccountsPurchase(Id, DetailId, DebitAccount, 0, AmountCashDiscountS, CreditAccount, VoucherType, VoucherNumber, VoucherDate, Narration, VoucherSubType, CreatedBy, CreatedDate, CreatedTime);
        //}
        //if (bRetValue == true && AmountSchemeDiscountS > 0)
        //{
        //    DetailId = Guid.NewGuid().ToString().ToUpper().Replace("-", "");
        //    DebitAccount = FixAccounts.AccountSchemeDiscountPurchcase;
        //    bRetValue = dbpur.AddDetailsForAccountsPurchase(Id, DetailId, DebitAccount, 0, AmountSchemeDiscountS, CreditAccount, VoucherType, VoucherNumber, VoucherDate, Narration, VoucherSubType, CreatedBy, CreatedDate, CreatedTime);
        //}
        //        if (bRetValue == true && AmountAddOnFreightS > 0)
        //        {
        //            DetailId = Guid.NewGuid().ToString().ToUpper().Replace("-", "");
        //            DebitAccount = FixAccounts.AccountAddonOctroiHamaliPurchase;
        //            bRetValue = dbpur.AddDetailsForAccountsPurchase(Id, DetailId, DebitAccount, AmountAddOnFreightS, 0, CreditAccount, VoucherType, VoucherNumber, VoucherDate, Narration, VoucherSubType, CreatedBy, CreatedDate, CreatedTime);
        //        }
        //        if (bRetValue == true && AmountLessS > 0)
        //        {
        //            DetailId = Guid.NewGuid().ToString().ToUpper().Replace("-", "");
        //            DebitAccount = FixAccounts.AccountLessPurchase;
        //            bRetValue = dbpur.AddDetailsForAccountsPurchase(Id, DetailId, DebitAccount, 0, AmountLessS, CreditAccount, VoucherType, VoucherNumber, VoucherDate, Narration, VoucherSubType, CreatedBy, CreatedDate, CreatedTime);
        ...
        //            bRetValue = dbpur.AddDetailsForAccountsPurchase(Id, DetailId, DebitAccount, RoundUpAmountS, 0, CreditAccount, VoucherType, VoucherNumber, VoucherDate, Narration, VoucherSubType, CreatedBy, CreatedDate, CreatedTime);
        //        }
        //        else if (bRetValue == true && RoundUpAmountS < 0)
        //        {
        //            DetailId = Guid.NewGuid().ToString().ToUpper().Replace("-", "");
        //            DebitAccount = FixAccounts.AccountRoundoffPurchase;
        //            bRetValue = dbpur.AddDetailsForAccountsPurchase(Id, DetailId, DebitAccount, 0, (RoundUpAmountS * -1), CreditAccount, VoucherType, VoucherNumber, VoucherDate, Narration, VoucherSubType, CreatedBy, CreatedDate, CreatedTime);
        //        }
        //        if (bRetValue == true && AmountCreditNoteS > 0)
        //        {
        //            DetailId = Guid.NewGuid().ToString().ToUpper().Replace("-", "");
        //            DebitAccount = FixAccounts.AccountCreditNotePurchase;
        //            bRetValue = dbpur.AddDetailsForAccountsPurchase(Id, DetailId, DebitAccount, AmountCreditNoteS, 0, CreditAccount, VoucherType, VoucherNumber, VoucherDate, Narration, VoucherSubType, CreatedBy, CreatedDate, CreatedTime);
        //        }
        //        if (bRetValue == true && AmountDebitNoteS > 0)
        //        {
        //            DetailId = Guid.NewGuid().ToString().ToUpper().Replace("-", "");
        //            DebitAccount = FixAccounts.AccountDebitNotePurchase;
        //            bRetValue = dbpur.AddDetailsForAccountsPurchase(Id, DetailId, DebitAccount, 0, AmountDebitNoteS, CreditAccount, VoucherType, VoucherNumber, VoucherDate, Narration, VoucherSubType, CreatedBy, CreatedDate, CreatedTime);
        //        }
        //        if (bRetValue == true && AmountNetS > 0)
        //        {
        //            DetailId = Guid.NewGuid().ToString().ToUpper().Replace("-", "");
        //            bRetValue = dbpur.AddDetailsForAccountsPurchase(Id, DetailId, CreditAccount, 0, AmountNetS, PurchaseAccount, VoucherType, VoucherNumber, VoucherDate, Narration, VoucherSubType, CreatedBy, CreatedDate, CreatedTime);
        //        }
        //    }
        //    catch (Exception Ex)
        //    {
        ...
        public int GetCurrentClosingStock(string thisStockID)
        {
            ...
            try
            {
                DBSsStock sstk = new DBSsStock();
                DataRow drow = null;
         //ss18-10       drow = sstk.GetCurrentClosingStockByThisStockID(thisStockID);
                if (drow != null)
                {
        ...
        public bool CheckForUniqueBillNumberforNew(string ChallanNumber, string accid)
        {
            ...
            try
            {
                DBChallanPurchase purb = new DBChallanPurchase();
                retVal = purb.CheckforUniqueBillNumberforNew(ChallanNumber, accid);
            }
            catch (Exception Ex)
            {
                Log.WriteException(Ex);
        ...
        {
            DBChallanPurchase dbcrdb = new DBChallanPurchase();
            return dbcrdb.UpdateDetails(Id, AccountID, Narration, EntryDate, ChallanNumber, VoucherSeries, VoucherType, VoucherNumber, VoucherDate,
           PurchaseBillNumber, AmountNetS, AmountClearS, AmountBillS, AmountItemDiscountS, AmountSpecialDiscountS,
           SpecialDiscountPercentS, AmountCashDiscountS, CreditNoteDiscountPercentS, AmountSchemeDiscountS,
           AmountAddOnFreightS, AmountLessS, CashDiscountPercentageS, AmountCreditNoteS, AmountDebitNoteS, RoundUpAmountS,
           OctroiPercentageS, AmountOctroiS, PurchaseAmount5PercentVATS, AmountVAT0PercentS, AmountVAT5PercentS,
        ...
        public bool UpdatePurchaseDetails(string Purchaseid, string purchaseBillNumber)
        {
            DBChallanPurchase dbcrdb = new DBChallanPurchase();
            return dbcrdb.UpdatePurchaseDetails(Id, Purchaseid, purchaseBillNumber, ModifiedBy, ModifiedDate, ModifiedTime);
        }
        ...
        {
            DBChallanPurchase dbchalnpur = new DBChallanPurchase();
            return dbchalnpur.DeletePreviousRecords(Id);
        }
        ...
        {
            DBChallanPurchase dbChalPur = new DBChallanPurchase();
            return dbChalPur.GetOverviewData();
        }
        ...
        {
            DBCreditNoteStock dbStock = new DBCreditNoteStock();
            return dbStock.GetOverviewDataForTempPurchase();
        }
        ...
        {
            DBChallanPurchase dbChlnPur = new DBChallanPurchase();
            return dbChlnPur.GetOverviewDataForPartywiseBills(PartyID);
        }
        public DataTable GetOverviewDataForProductwiseBills(string PartyID)
        {
            DBChallanPurchase dbChlnPur = new DBChallanPurchase();
            return dbChlnPur.GetOverviewDataForProductwiseBills(PartyID);
        }
        public DataTable GetOverviewDataForPartywiseBills(string PartyID, string fromDate, string toDate)
        {
            DBChallanPurchase dbChlnPur = new DBChallanPurchase();
            return dbChlnPur.GetOverviewDataForPartywiseBills(PartyID, fromDate, toDate);
        }
        ...
        public DataTable GetOverviewDataForLBTPurchaseOUT(string mFromDate, string mToDate)
        {
            DBProduct dbprod = new DBProduct();
            return dbprod.GetOverviewDataForLBTPurchaseOUT(mFromDate, mToDate);
        }

        public DataTable GetOverviewDataForLBTPurchaseWITHIN(string mFromDate, string mToDate)
        {
            DBProduct dbprod = new DBProduct();
            return dbprod.GetOverviewDataForLBTPurchaseWITHIN(mFromDate, mToDate);
        }
        public DataTable GetOverviewDataForLBTPurchasePartywise(string partyID, string mFromDate, string mToDate)
        {
            DBProduct dbprod = new DBProduct();
            return dbprod.GetOverviewDataForLBTPurchasePartywise(partyID, mFromDate, mToDate);
        }
        public double GetOverviewDataForLBTSummaryTotalPurchase(string mFromDate, string mToDate)
        {
            double amt = 0;
            DBProduct dbprod = new DBProduct();
            DataRow dr = dbprod.GetOverviewDataForLBTSummaryTotalPurchase(mFromDate, mToDate);
            if (dr != null && dr["AmountNet"] != DBNull.Value)
                amt = Convert.ToDouble(dr["AmountNet"].ToString());
            return amt;
        }
        public double GetOverviewDataForLBTPurchaseOUTSummary(string mFromDate, string mToDate)
        {
            double amt = 0;
            DBProduct dbprod = new DBProduct();
            DataRow dr = dbprod.GetOverviewDataForLBTPurchaseOUTSummary(mFromDate, mToDate);
            if (dr != null && dr["AmountNet"] != DBNull.Value)
                amt = Convert.ToDouble(dr["AmountNet"].ToString());
            return amt;
        }
        public double GetOverviewDataForLBTPurchaseINSummary(string mFromDate, string mToDate)
        {
            double amt = 0;
            DBProduct dbprod = new DBProduct();
            DataRow dr = dbprod.GetOverviewDataForLBTPurchaseINSummary(mFromDate, mToDate);
            if (dr != null && dr["AmountNet"] != DBNull.Value)
                amt = Convert.ToDouble(dr["AmountNet"].ToString());
            return amt;
        }
        ...
        //{
        //    DBPurchase dbp = new DBPurchase();
        //    dbp.UpdateTempPurchasewithVoucherID(iD, tempID, quantity);
        //}
        ...
        internal int GetLastVoucherNumber(string vouType, string subType, string vouSeries)
        {
            DataRow dr;
            int lastvouno = 0;
            try
