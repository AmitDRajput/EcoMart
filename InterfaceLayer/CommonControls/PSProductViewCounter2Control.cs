...
{
    public partial class PSProductViewCounter2Control : UserControl
    {
        ...
        public PSProductViewCounter2Control()
        {
            InitializeComponent();
        ...
        public void BindGridProductList()
        {
            try
            {
                if (DataSourceProductList != null)
                {
                    ...
                    DataColumn Col = DataSourceProductList.Columns.Add("CounterID", System.Type.GetType("System.Int32"));
                    Col.SetOrdinal(1);
        ...
        private DataTable MergeRowsWithBatchQuantity(DataTable dtTempCounterSale) // Updated on [11.5.2017] [Ansuman]
        {
        ...
        private Point GetProductGridLocation()
        {
            ...
            try
            {
                pt.X = dgMainGrid.Location.X + dgMainGrid.RowHeadersWidth + dgMainGrid.Columns["Col_ProductName"].Width;
        ...
        private string GetFilterString(string filterValue)
        {
            ...
            try
            {
                ...
                strFilterColumn = dgMainGrid.Columns["Col_ProductName"].DataPropertyName;
                if (strFilterString != "")
                    strFilterString += " AND ";
                if (DataSourceProductList.Columns[dgMainGrid.Columns["Col_ProductName"].DataPropertyName].DataType == typeof(int))
                {
                    strFilterString += strFilterColumn + " = ";
                    strFilterString += filterValue + " ";
                }
                else if (DataSourceProductList.Columns[dgMainGrid.Columns["Col_ProductName"].DataPropertyName].DataType == typeof(string))
                {
                    strFilterString += strFilterColumn;
                    if (General.CurrentSetting.MsetGeneralAlphabetical == "Y")
        ...
        public void SetFocus(int columnIndex)
        {
            try
            {
                ...
                {
                    string currentCounterId = "1";

                    if (dgMainGrid.CurrentRow.Cells[1].Value != null)
                        currentCounterId = dgMainGrid.CurrentRow.Cells[1].Value.ToString();

                    dgMainGrid.SetCurrentCell(dgMainGrid.CurrentRow.Index, columnIndex, Convert.ToInt32(dgMainGrid.CurrentRow.Cells["Col_CounterID"].Value));

                    if (dgMainGrid.CurrentRow.Cells["Col_CounterID"].Value == null)
                        dgMainGrid.CurrentRow.Cells["Col_CounterID"].Value = currentCounterId;
                }
                //ss 20/3/2013
        ...
        public DataGridViewRow MainDataGridNextRow  // [01.12.2016]
        {
            get { return dgMainGrid.Rows[dgMainGrid.CurrentRow.Index+1]; }
        }
        ...
        private bool dgMainGrid_EnterKeyPressed(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dgMainGrid.IsCounterIDColumn())//CounterID Column in the Main Grid
                {
                   
                    switch(Convert.ToString(dgMainGrid.CurrentCell.Value))
                    {
                        case "1":
                            dgMainGrid.CurrentRow.DefaultCellStyle.BackColor = Color.LightSalmon;
                            break;
                        case "2":
                            dgMainGrid.CurrentRow.DefaultCellStyle.BackColor = Color.Cyan;
                            break;
                        case "3":
                            dgMainGrid.CurrentRow.DefaultCellStyle.BackColor = Color.Pink;
                            break;
                        case "4":
                            dgMainGrid.CurrentRow.DefaultCellStyle.BackColor = Color.Lime;
                            break;
                        case "5":
                            dgMainGrid.CurrentRow.DefaultCellStyle.BackColor = Color.DarkKhaki;
                            break;
                        case "6":
                            dgMainGrid.CurrentRow.DefaultCellStyle.BackColor = Color.PaleGoldenrod;
                            break;

                    }
                    if(string.IsNullOrEmpty(Convert.ToString(dgMainGrid.CurrentRow.Cells[2].Value)) == true)                   
                       SetFocus(2);
                }
            }
            catch (Exception ex) { Log.WriteError(ex.ToString()); }
        ...
        private void dgMainGrid_OnCellValueChangeCommited(int colIndex)
        {
            try
            {
                ...
                //}
                //else
                if (string.IsNullOrWhiteSpace(Convert.ToString(dgMainGrid.CurrentRow.Cells[2].Value)) == false)
                {
                    FillBarCodeProduct();
                }
            }
        ...
        private void dgMainGrid_OnArrowUpDownPressed(int keyValue)
        {
            try
            {
                ...
                else if (dgMainGrid.CurrentCell.ColumnIndex == dgMainGrid.CurrentRow.Cells["Col_CounterID"].ColumnIndex || dgMainGrid.CurrentCell.ColumnIndex == dgMainGrid.CurrentRow.Cells["Col_ProductName"].ColumnIndex)
                {
        ...
        private void dgMainGrid_OnRowAdded(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    OnRowAdded(sender, e);
                    dgMainGrid.CurrentRow.Cells[2].ReadOnly = true;   //  [ansuman] [01.12.2016]
                    dgMainGrid.CurrentRow.Cells[12].ReadOnly = false; //  [ansuman] [01.12.2016]
                    General.ProdID = "";   //  [ansuman] [2.11.2016]
                    SetFocus(2);  // Temporary
                }
            }
            catch (Exception ex)
            {
