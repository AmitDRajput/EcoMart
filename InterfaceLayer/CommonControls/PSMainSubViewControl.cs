...
{
    public partial class PSMainSubViewControl : UserControl
    {
        #region Declaration
        ...
        public delegate void CellEnterKeyPressed(int SelectedCellIndex);
        public event CellEnterKeyPressed OnCellEnterKeyPressed;
            ...
            try
            {
                ...
                else if (DataSource.Columns[dgMainGrid.Columns[1].DataPropertyName].DataType == typeof(string))
                {
                    ...
                    if (dtMergedProduct.Rows.Count == 0)
                        _BindingSource.DataSource = DataSource;     // [ansuman] [for DebitNoteExpiry, searching for partynames][3.1.2017]
                    dgSubGrid.DataSource = _BindingSource;
                    // ss 20/5/2014
                    if (dgSubGrid.Columns.Count > 0)
                        dgSubGrid.Columns[0].Visible = false;
                }
                if (strFilterColumn.Equals("ProdName") == true && DataSource.Columns.Contains("Barcode") == true)
                {
        ...
        //kiran
        public bool VisibleProductGrid()
        {
            bool retValue = false;
        ...
        private void dgMainGrid_EnterKeyPressed(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dgMainGrid.IsFirstColumn())
                {
                    ...
                    {
                        dgSubGrid.Visible = false;
                        if (string.IsNullOrEmpty(Convert.ToString(dgMainGrid.CurrentRow.Cells[0].Value)) == true)
                            dgMainGrid.CurrentCell.Value = "";
                        SetFocus(dgMainGrid.CurrentCell.ColumnIndex);
                    }
                }
                if (OnCellEnterKeyPressed != null)
                    OnCellEnterKeyPressed(dgMainGrid.CurrentCell.ColumnIndex);
            }
            catch (Exception ex) { Log.WriteError(ex.ToString()); }
        }
        ...
        private void dgMainGrid_OnArrowLeftRightPressed(int keyValue)
        {
            try
            {
                if (dgSubGrid.Visible && dgSubGrid.Rows.Count > 0)
