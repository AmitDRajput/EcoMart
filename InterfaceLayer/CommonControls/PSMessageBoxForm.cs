...
namespace EcoMart.InterfaceLayer.CommonControls
{
    public partial class PSMessageBoxForm : Form
    {
        ...
        public void Show(string messageLine1, string messageLine2, string title, PSMessageBoxButtons button, PSMessageBoxIcon icon, PSMessageBoxButtons focusButton, double netAmount)
        {
            try
            {
                ...
                txtTotalAmt.Text = Convert.ToString(netAmount);

                if (General.CurrentSetting.MsetSaleTenderAmount == "Y")
                {
                    pnlTenderAmount.Visible = true;
                    this.Height = 255;
                    buttonToFocus = PSMessageBoxButtons.None;
                }
                else
                    buttonToFocus = PSMessageBoxButtons.OK;
        ...
        private void txtAmountRcvd_KeyUp(object sender, KeyEventArgs e)
        {

            try
            {
                double tenderAmt = Convert.ToDouble(txtReceivedAmt.Text) - Convert.ToDouble(txtTotalAmt.Text);
                txtTenderAmt.Text = Convert.ToString(Math.Round(tenderAmt, 2, MidpointRounding.AwayFromZero));
