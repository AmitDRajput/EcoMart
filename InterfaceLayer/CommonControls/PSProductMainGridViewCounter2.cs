...
namespace EcoMart.InterfaceLayer.CommonControls
{
    ...
    internal class PSProductMainGridViewCounter2 : DataGridView
    {
        ...
        public PSProductMainGridViewCounter2()
            : base()
        {
        ...
        private void ctl_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            else if (this.CurrentCell.OwningColumn.Name == "Col_CounterID")
            {
                if ((!char.IsControl(e.KeyChar)
                   && !char.IsDigit(e.KeyChar))
                   || e.KeyChar == '.')
        ...
        public void Initialize()
        {
            if (this.Columns.Count > 0 && this.Rows.Count == 0)
            {
                ...
                SetCurrentCellForFirstTime(newRowId, 0); // [02.12.2016]
            }
        ...
        public void SetCurrentCell(int rowID, int cellID, int counterID)
        {
            try
            {
                for (int colIndex = cellID; colIndex < this.Columns.Count; colIndex++)
                {
                    ...
                    {
                        if ((this.Rows[rowID].Cells[colIndex].Visible && this.Rows[rowID].Cells[colIndex].ReadOnly == false) || (colIndex == 2))
                        {

                            this.EndEdit();
                            //this.Rows[rowID].Cells[colIndex].ReadOnly = false;
                            this.CurrentCell = this.Rows[rowID].Cells[colIndex];
                            this.BeginEdit(true);
                            if (CurrentCell.ColumnIndex == 1)       // [temporary]
                                this.CurrentCell.Value = counterID;
                            if (CurrentCell.ColumnIndex == 2)
                                this.Rows[rowID].Cells[colIndex - 1].Value = counterID;
                            break;
                        }
                    }
                    else
            ...
            try
            {
                ...
                for (int colIndex = cellID; colIndex < this.Columns.Count; colIndex++)
                {
                    if (this.Rows[rowID] != null)
                    {
                        ...
                        if (colIndex == 1)
                        {
                            this.CurrentRow.Cells[1].Value = "3";
        ...
        public void SetCurrentCellForFirstTime(int rowID, int cellID)  // [02.11.2016]
        {
            try
            {
                for (int colIndex = cellID; colIndex < this.Columns.Count; colIndex++)
                {
                    if (this.Rows[rowID] != null)
                    {
                        if ((this.Rows[rowID].Cells[colIndex].Visible && this.Rows[rowID].Cells[colIndex].ReadOnly == false) || (colIndex == 2))
                        {
                            ...
                            this.CurrentCell = this.Rows[rowID].Cells[colIndex];
                            this.BeginEdit(true);
                            if (CurrentCell.ColumnIndex == 1 && rowID == 0)
                                this.CurrentCell.Value = 1;  // [02.12.2016]
                            break;
                        }
                        if (colIndex == 1)
                            this.CurrentRow.Cells[1].Value = "3";
        ...
        public void addNewBlankRow()
        {
            ...
            if (this.CurrentRow != null)
                counterId = Convert.ToInt32(this.CurrentRow.Cells[1].Value);

            int newRowId = this.Rows.Add();

            if (OnRowAdded != null)
                OnRowAdded(newRowId, new EventArgs());
            SetCurrentCell(newRowId, 2, counterId);
        }
            ...
            try
            {
                ...
                if (key.KeyCode == Keys.Enter)
                {
                    ...
                    if (this.CurrentCell.Value != null && this.CurrentCell.Value.ToString() != "")
                    {
                        ...
                        {
                            int counterId = Convert.ToInt32(this.CurrentRow.Cells[1].Value);
                            System.Drawing.Color currentRowColor = (System.Drawing.Color)CurrentRow.DefaultCellStyle.BackColor;
                            int newRowId = this.Rows.Add();
                            if (OnRowAdded != null)
                                OnRowAdded(newRowId, new EventArgs());
                            SetCurrentCell(newRowId, 2, counterId);

                            CurrentRow.DefaultCellStyle.BackColor = currentRowColor;
                        }
                    ...
                    if (canrowdelete)
                    {
                        ...
                        if (rowIndex == this.Rows.Count) //If Deleted last row
                        {
                            ...
                            else
                            {
                                ...
                                this.Rows[newRowId].Cells[2].ReadOnly = false;
                                SetCurrentCell(newRowId, 2, 1);
                            }
                        }
                        else if (rowIndex == 0)
                        {
        ...
        {
            return (this.CurrentCell.ColumnIndex == 2);
        }

        public bool IsCounterIDColumn()
        {
