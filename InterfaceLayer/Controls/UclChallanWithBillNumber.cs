...
namespace PharmaSYSDistributorPlus.InterfaceLayer
{
    ...
    public partial class UclChallanWithBillNumber : BaseControl
    {
        #region Declaration
        ...
        public UclChallanWithBillNumber()
        {
            InitializeComponent();
        ...
        private bool UpdatePurchaseID()
        {
            ...
            foreach (DataGridViewRow dr in dgChallanPurchase.Rows)
            {
                if (string.IsNullOrEmpty(Convert.ToString(dr.Cells["Col_ChallanID"].Value)) == false
                              ...
                              && dr.Cells["Col_AmountNet"].Value != null && dr.Cells["Col_AmountNet"].Value.ToString().Trim() != "")
                {
        ...
        private bool SaveParticularsProductwise()
        {
            ...
            try
            {
                foreach (DataGridViewRow prodrow in mpMSVC.Rows)
                {
                  ...
                  //ss 18-10      ThisStockID = _Purchase.CheckForBatchMRPStockIDInStockTable();
                        ...
                        if (ThisStockID == string.Empty)
                        {
                            ...
                            //Create new scancode
                        // ss 18-10    returnVal = _Purchase.AddProductDetailsInStockTable();
                        }
                        else
                        {
        ...
        private DataTable FillChallanPurchaseDetail()
        {
            ...
            try
            {
                ...
                if (mcbCreditor.SelectedID != null && mcbCreditor.SelectedID != "")
                {
                    ...
                    dt = dbChlnpur.GetOverviewDataForPartywiseBills(mcbCreditor.SelectedID);
                    if (dt != null)
        ...
        private bool BindChallanPurchaseDetail(DataTable dt)
        {
            ...
            try
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    currentdr.Cells["Col_ChallanID"].Value = dr["ID"].ToString();
        ...
        private void FillPurchaseDetailData()
        {
            ...
            foreach (DataGridViewRow dr in dgChallanPurchase.Rows)
            {
               ...
               && dr.Cells["Col_AmountNet"].Value != null && dr.Cells["Col_AmountNet"].Value.ToString().Trim() != "")
                {
                    _ChallanPurchase.Id = Convert.ToString(dr.Cells["Col_ChallanID"].Value);
                    dt = _ChallanPurchase.ReadProductDetailWithoutPurchaseByID();
        ...
        private void FillDataPurDeatil(DataTable dt)
        {
            foreach (DataRow item in dt.Rows)
            {
                ...
                mpMSVC.Rows[rowindex].Cells["Col_DetailPurchaseID"].Value = setdata(item["DetailPurchaseID"]);
                mpMSVC.Rows[rowindex].Cells["Col_ProductID"].Value = setdata(item["ProductID"]);
                mpMSVC.Rows[rowindex].Cells["Col_ProductName"].Value = setdata(item["ProdName"]);
                mpMSVC.Rows[rowindex].Cells["Col_Company"].Value = setdata(item["ProdCompID"]);

                mpMSVC.Rows[rowindex].Cells["Col_UnitOfMeasure"].Value = setdata(item["Prodloosepack"]);
                mpMSVC.Rows[rowindex].Cells["Col_Pack"].Value = setdata(item["prodpack"]);

                mpMSVC.Rows[rowindex].Cells["Col_Quantity"].Value = setdata(item["Quantity"]);
                mpMSVC.Rows[rowindex].Cells["Col_BatchNumber"].Value = setdata(item["BatchNumber"]);
                mpMSVC.Rows[rowindex].Cells["Col_Expiry"].Value = setdata(item["Expiry"]);
                mpMSVC.Rows[rowindex].Cells["Col_ExpiryDate"].Value = setdata(item["ExpiryDate"]);
                mpMSVC.Rows[rowindex].Cells["Col_TradeRate"].Value = setdata(item["TradeRate"]);
                mpMSVC.Rows[rowindex].Cells["Col_MRP"].Value = setdata(item["MRP"]);
                mpMSVC.Rows[rowindex].Cells["Col_PurchaseRate"].Value = setdata(item["PurchaseRate"]);
                mpMSVC.Rows[rowindex].Cells["Col_SaleRate"].Value = setdata(item["SaleRate"]);
                mpMSVC.Rows[rowindex].Cells["Col_VAT"].Value = setdata(item["PurchaseVATPercent"]);
                mpMSVC.Rows[rowindex].Cells["Col_VATAmountPurchase"].Value = setdata(item["AmountPurchaseVAT"]);
                mpMSVC.Rows[rowindex].Cells["Col_Scheme"].Value = setdata(item["SchemeQuantity"]);
                mpMSVC.Rows[rowindex].Cells["Col_Replacement"].Value = setdata(item["ReplacementQuantity"]);
                mpMSVC.Rows[rowindex].Cells["Col_ItemDiscountPer"].Value = setdata(item["ItemDiscountPercent"]);
                mpMSVC.Rows[rowindex].Cells["Col_Amount"].Value = setdata(item["Amount"]);
                mpMSVC.Rows[rowindex].Cells["Col_ItemDiscountAmount"].Value = setdata(item["AmountItemDiscount"]);
                mpMSVC.Rows[rowindex].Cells["Col_ItemSCMDiscountAmount"].Value = setdata(item["AmountSchemeDiscount"]);
                mpMSVC.Rows[rowindex].Cells["Col_CSTAmount"].Value = setdata(item["AmountCST"]);
                mpMSVC.Rows[rowindex].Cells["Col_ProdVATPer"].Value = setdata(item["ProdVATPercent"]);
                mpMSVC.Rows[rowindex].Cells["Col_VATAmountSale"].Value = setdata(item["AmountProdVAT"]);
                mpMSVC.Rows[rowindex].Cells["Col_SpldiscountPer"].Value = setdata(item["SchemeDiscountPercent"]);
                mpMSVC.Rows[rowindex].Cells["Col_SplDiscountAmount"].Value = setdata(item["AmountSpecialDiscount"]);
                mpMSVC.Rows[rowindex].Cells["Col_CashDiscountAmount"].Value = setdata(item["AmountCashDiscount"]);
                mpMSVC.Rows[rowindex].Cells["Col_Margin"].Value = setdata(item["Margin"]);
                mpMSVC.Rows[rowindex].Cells["Col_Margin2"].Value = setdata(item["MarginAfterDiscount"]);
                // mpMSVC.Rows[rowindex].Cells["Col_SplDiscountAmount"].Value = txtSplDiscountPerUnit.Text.ToString();
                mpMSVC.Rows[rowindex].Cells["Col_StockID"].Value = setdata(item["StockID"]);
                mpMSVC.Rows[rowindex].Cells["Col_ScanCode"].Value = setdata(item["ScanCode"]);

                mpMSVC.Rows[rowindex].Cells["Col_ShelfID"].Value = setdata(item["ShelfCode"]);
                mpMSVC.Rows[rowindex].Cells["Col_DistributorSaleRate"].Value = setdata(item["DistributorSaleRate"]);
                mpMSVC.Rows[rowindex].Cells["Col_DistributorSaleRatePer"].Value = setdata(item["DistributorSaleRatePer"]);
        ...
        private void CalculateTotals()
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow dr in dgChallanPurchase.Rows)
                {
                    ...
                    {
                        itemCount += 1;
                        double.TryParse(dr.Cells["Col_AmountNet"].Value.ToString(), out mamt);
                        mtotamt += mamt;
                    }
