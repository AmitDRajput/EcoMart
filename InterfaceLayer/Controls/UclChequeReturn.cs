        ...
        public override bool Save()
        {
            ...
            try
            {
                if (txtAmountReceived.Text != null && txtAmountReceived.Text != "")
                {
                    ...
                    _ChequeReturn.CBChequeReturnDate = dtpReturnDate.Value.Date.ToString("yyyyMMdd");
                    ...
                    if (_ChequeReturn.IsValid)
                    {
                        if (_Mode == OperationMode.Edit)
                        {
                           ...
                           //_ChequeReturn.CBVouNo = _ChequeReturn.GetAndUpdateChequreReturn(General.ShopDetail.ShopVoucherSeries);
                            ...
                            //retValue = _ChequeReturn.AddDetails();_BankReceipt.IntID = 0;
                            _ChequeReturn.IntID = _ChequeReturn.AddDetails();
                            if (_ChequeReturn.IntID > 0)
                                retValue = true;
                            else
                                retValue = false;
                        ...
                        else if (_Mode == OperationMode.Fifth)
                        {
                            ...
                            retValue = _ChequeReturn.UpdateReturnDate();
                ...
                try
                {
                    foreach (DataGridViewRow drow in mpMSVC.Rows)
                    {
                           ...
                           
                        {
                            ...
                            if (_ChequeReturn.DSaleId == "OPB")
                            {
                                _ChequeReturn.DSaleId = "0";
                            }
                            else
                            {
                                _ChequeReturn.DSaleId = drow.Cells["Col_ID"].Value.ToString();
        ...
        public override bool FillSearchData(string ID, string Vmode)
        {
            try
            {
                if (ID != null && ID != "")
                {
                    ...
                    dtpReturnDate.Value = General.ConvertStringToDateyyyyMMdd(_ChequeReturn.CBChequeReturnDate);
                    ...
                    else if (_ChequeReturn.CBIfChequeReturn == "Y")
                    {
