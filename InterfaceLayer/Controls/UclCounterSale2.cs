...
namespace PharmaSYSDistributorPlus.InterfaceLayer
{
    ...
    public partial class UclCounterSale2 : BaseControl
    {
        #region Declaration
        ...
        // string _preID = "";
        public PSProductViewCounter2Control ActiveDataGrid;
        ...
        private void UclCounterSale2_Load(object sender, EventArgs e)
        {
      ...
      private void SetGridRowColour()
        {
            try
            {
                if (ActiveDataGrid != null && ActiveDataGrid.Rows.Count > 0 && ActiveDataGrid.MainDataGridCurrentRow.Cells["Col_CounterID"].Value == null || ActiveDataGrid.MainDataGridCurrentRow.Cells["Col_CounterID"].Value.ToString() == string.Empty)
                {
        ...
        {
            if (ActiveDataGrid.MainDataGridCurrentRow.Cells[1].Value != null)
            {
                if (txtsavecustno.Text.ToString().Trim() == "1")
                {
                    //ActiveGrid.MainDataGridCurrentRow.DefaultCellStyle.BackColor = btn1.BackColor;
        ...
        private bool SaveData(bool printData)
        {
            ...
            if (txtsavecustno.Text != null && Convert.ToInt32(txtsavecustno.Text.ToString()) > 0)
            {
                try
                {
                    ...
                    {
                        if (General.ShopDetail.ShopChangeCounterSaleType != "Y" && _SSSale.CrdbVouType == FixAccounts.VoucherTypeForVoucherSale)
                        {
        ...
        //    CalculateAmount(-1, txtsavecustno.Text.Trim());
        ...
        //                if (General.CurrentSetting.MsetSaleProductRefreshButton == "N")
        //                    btnRefreshClick();
        //            }
        //            else
        //            {
        ...
        private void BindActiveDataGrid()
        {
        ...
        private void DeleteRecordsForSelectedNumber()
        {
            try
            {
                ...
                {
                    if (prodrow.Cells["Col_CounterID"].Value.ToString() == txtsavecustno.Text)
        ...
        private bool SaveParticularsProductwise()
        {
            ...
            try
            {
                ...
                {
                    if (prodrow.Cells["Col_ProductName"].Value != null &&
                       Convert.ToDouble(prodrow.Cells["Col_Quantity"].Value) > 0 && prodrow.Cells["Col_CounterID"].Value.ToString() == txtsavecustno.Text)
                    {
        ...
        private void ActiveDataGridCellValueChangeCommited(int colIndex)
        {
            ...
            try
            {
                ...
                //    for(int i=0; i<6;i++)
                //    {
                //        if(Convert.ToInt32(ActiveGrid.MainDataGridCurrentRow.Cells["Col_CounterID"].Value)==i)
                //        {
                //            _lastCustIdSelected = i.ToString();
                //            txtsavecustno.Text = i.ToString();
                //        }
                //    }
                //}
                if (colIndex == 1)
                {
                    ...
                    //}
                    txtsavecustno.Text = ActiveDataGrid.MainDataGridCurrentRow.Cells["Col_CounterID"].Value.ToString();
                }
                                ...
                                else
                                {
                                    ...
                                    if (requiredQty <= 0 || _SSSale.CurrentBatchStock == 0)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if (ActiveDataGrid.MainDataGridCurrentRow.Cells["Col_CustID"].Value != null)
                                                counterId = ActiveDataGrid.MainDataGridCurrentRow.Cells["Col_CounterID"].Value.ToString();

                                            lblFooterMessage.Text = "";
                                            lblRightSideFooterMsg.Text = "";
                                            txtMinlevel.Text = "";
                                            txtMaxlevel.Text = "";
                                            FillBatchStock(ref mmrp, ref mrate, ref mpakn, ref mbtno, ref mprodno, ref mcurrentindex, ref oldmqty, ref mqty, ref custno, counterId, requiredQty);
                                    ...
                                    else
                                    {
                                        ...
                                        if (requiredQty <= _SSSale.CurrentBatchStock || (ActiveDataGrid.Rows.Count == currindex + 1) || (ActiveDataGrid.Rows.Count > currindex + 1 && (ActiveDataGrid.Rows[currindex + 1].Cells["Col_ProductID"].Value == null)))
                                        {
                                            ...
                                            if (ActiveDataGrid.MainDataGridCurrentRow.Cells["Col_CounterID"].Value != null)
                                                counterId = ActiveDataGrid.MainDataGridCurrentRow.Cells["Col_CounterID"].Value.ToString();
                                            ...
                                            {
                                                if (requiredQty > (_SSSale.CurrentBatchStock - activegridbatchstock))
                                                    requiredQty = (_SSSale.CurrentBatchStock - activegridbatchstock);
                                            ...
                                            else
                                            {
                                                ...
                                                FillMainGridwithMultipleBatch(requiredQty, mprodno, checkCounterDt, mbtno, counterId);
                                                CalculateAmount(-1);
                                                //ActiveDataGrid.ColumnsMain["Col_Quantity"].ReadOnly = true;
        ...
        private void FillMainGridwithMultipleBatch(int requiredqty, string productID, bool checkCounterDt, string batchId, string counterId)
        {
            int mmaingridrowIndex = 0;
            DataTable stkdt = new DataTable();
            Stock prodstk = new Stock();
            ...
            try
            {
                ...
                foreach (DataRow dtrow in stkdt.Rows)
                {
                    ...
                    if (mactualsalestock > 0 && msalestk > 0 && mactualclosingstock > 0)
                    {
                        ...
                        ActiveDataGrid.Rows[mycolindex].Cells["Col_CounterID"].Value = counterId;
                        ActiveDataGrid.Rows[mycolindex].Cells["Col_ProductID"].Value = dtrow["ProductID"].ToString();
                        ActiveDataGrid.Rows[mycolindex].Cells["Col_ProductName"].Value = dtrow["ProdName"].ToString();
                        ActiveDataGrid.Rows[mycolindex].Cells["Col_UOM"].Value = dtrow["ProdLoosePack"].ToString();
                        ActiveDataGrid.Rows[mycolindex].Cells["Col_Pack"].Value = dtrow["ProdPack"].ToString();
                        ...
                        if (msalestk > 0 && mactualclosingstock > 0)
                        {
                            ...
                            ActiveDataGrid.SetFocus(ActiveDataGrid.Rows.Count - 1, 12);
                            mmaingridrowIndex = mmaingridrowIndex + 1;
                        }
                    }
                }
        ...
        private void FillBatchStock(ref double mmrp, ref double mrate, ref int mpakn, ref string mbtno, ref string mprodno, ref int mcurrentindex, ref int oldmqty, ref int mqty, ref int custno, string CounterId, int requiredQty)
        {
            mcurrentindex = ActiveDataGrid.MainDataGridCurrentRow.Index;
            if (ActiveDataGrid.MainDataGridCurrentRow.Cells["Col_ProductID"].Value != null)
                mprodno = ActiveDataGrid.MainDataGridCurrentRow.Cells["Col_ProductID"].Value.ToString().Trim();
            ...
            foreach (DataGridViewRow drp in ActiveDataGrid.Rows)
            {
                         ...
                         drp.Cells["Col_MRP"].Value != null)
                {
                    ...
                    if (drp.Cells["Col_ProductID"].Value.ToString().Trim() == mprodno &&
                          drp.Cells["Col_BatchNumber"].Value.ToString().Trim() == mbtno &&
                             drp.Cells["Col_MRP"].Value.ToString().Trim() == mmrp.ToString("#0.00") && drp.Index != mcurrentindex
                             && drp.Cells["Col_CounterID"].Value.ToString().Trim() == CounterId)
                    {
                        if (drp.Cells["Col_Quantity"].Value != null)
                            oldmqty = Convert.ToInt32(drp.Cells["Col_Quantity"].Value.ToString());
                        ...
                        ActiveDataGrid.MainDataGridCurrentRow.Cells["Col_Quantity"].Value = mqty;
                }
            }
        ...
        private void ActiveDataGridOnRowDeleted(object sender)
        {
            try
            {
                ...
                { 
                    ...
                    CalculateAmount(deletedrowindex, deletedrow.Cells[1].Value.ToString());  // [01.12.2016]
        ...
        {
            //CalculateAmount(-1, txtsavecustno.Text);
            if (ActiveGrid.MainDataGridCurrentRow.Cells["Col_CounterID"].Value != null)
                CalculateAmount(-1, ActiveGrid.MainDataGridCurrentRow.Cells["Col_CounterID"].Value.ToString());
            ...
            if (savingcusttotal > 0 && ((string.IsNullOrEmpty(Convert.ToString(ActiveGrid.MainDataGridCurrentRow.Cells["Col_ProductName"].Value)) == true) && (ActiveGrid.MainDataGridCurrentRow.Cells["Col_ProductID"].Value == null && ActiveDataGrid.MainDataGridCurrentRow.Cells["Col_Amount"].Value == null) || (General.CurrentSetting.MsetPrintFixNumberOfLines == "Y")))
            {
                ...
                {
                    if (ActiveGrid.MainDataGridCurrentRow.Cells["Col_CounterID"].Value != null)
                        ShowHideActiveGridRows(ActiveGrid.MainDataGridCurrentRow.Cells["Col_CounterID"].Value.ToString());
                    ...
                    if (ActiveGrid.MainDataGridCurrentRow.Cells["Col_CounterID"].Value != null)
                        txtsavecustno.Text = ActiveGrid.MainDataGridCurrentRow.Cells["Col_CounterID"].Value.ToString();
                    else
                        txtsavecustno.Text = _lastCustIdSelected.ToString();
        ...
        private void ShowHideActiveGridRows(string counterId)
        {
            foreach (DataGridViewRow dr in ActiveDataGrid.Rows)
            {
                if (dr.Cells["Col_CounterID"].Value != null && dr.Cells["Col_CounterID"].Value.ToString() != counterId)
                {
                    dr.Visible = false;
                }
            }
        ...
        private double SavingCustomersTotalSale()
        {
            ...
            savingcusttot = Convert.ToDouble(ActiveDataGrid.Rows[ActiveDataGrid.Rows.Count - 2].Cells["Col_Amount"].Value);
        ...
        private void CalculateAmount(int deletedrowindex, string counterId)
        {
            ...
            try
            {
                if (ActiveDataGrid != null)
                {
                    foreach (DataGridViewRow dr in ActiveDataGrid.Rows)
                    {
                        ...
                        {
                            if ((dr.Cells["Col_CounterID"].Value != null && Convert.ToString(dr.Cells["Col_CounterID"].Value) == counterId.ToString()) && dr.Cells["Col_ProductID"].Value != null && dr.Cells["Col_Quantity"].Value != null && dr.Cells["Col_Quantity"].Value.ToString() != "0" && dr.Cells["Col_Quantity"].Value.ToString() != "")
                            {
                                ifdiscount = "Y";
                    ...
                    if (cbRound.Checked == true)
                    {
                        ...
                        txtNetAmount.Text = String.Format(new CultureInfo("en-IN"), "{0 :C2}", 123.45);
                    ...
                    if (counterId.ToString().Trim() == "1")
                    {
                        ...
                        txtNetAmount.Text = txtBillAmount.Text;
                        txtNetAmount.Text = String.Format(new CultureInfo("en-IN"), "{0 :C2}", 123.45);
                        double x = Convert.ToDouble(txtNetAmount.Text);
                        MessageBox.Show("Counter 1: " + x);
                        resetCounterTotalColor(1);
                    }
                    else if (counterId.ToString().Trim() == "2")
                    {
                        ...
                        txtNetAmount.Text = txtBillAmount.Text;
                        txtNetAmount.Text = String.Format(new CultureInfo("en-IN"), "{0 :C2}", 123.45);
                        resetCounterTotalColor(2);
                    }
                    else if (counterId.ToString().Trim() == "3")
                    {
                        ...
                        txtNetAmount.Text = txtBillAmount.Text;
                        txtNetAmount.Text = String.Format(new CultureInfo("en-IN"), "{0 :C2}", 123.45);
                        resetCounterTotalColor(3);
                    }
                    else if (counterId.ToString().Trim() == "4")
                    {
                        ...
                        txtNetAmount.Text = txtBillAmount.Text;
                        txtNetAmount.Text = String.Format(new CultureInfo("en-IN"), "{0 :C2}", 123.45);
                        resetCounterTotalColor(4);
                    }
                    else if (counterId.ToString().Trim() == "5")
                    {

                        txtamount5.Text = mTotalAmount.ToString("#0.00");
                        txtitems5.Text = itemCount.ToString();
                        txtBillAmount.Text = txtamount5.Text;
                        txtNetAmount.Text = txtBillAmount.Text;
                        txtNetAmount.Text = String.Format(new CultureInfo("en-IN"), "{0 :C2}", 123.45);
                        resetCounterTotalColor(5);
                    }
                    else
                    {
        ...
        private void resetCounterTotalColor(int currentId)
        {
            for (int i = 1; i <= 6; i++)
            {
                DecimalTextBox txt = (DecimalTextBox)this.Controls.Find(string.Format("txtamount{0}", i), true).FirstOrDefault();
                TextBox txtItems = (TextBox)this.Controls.Find(string.Format("txtitems{0}", i), true).FirstOrDefault();
                Button btn = (Button)this.Controls.Find(string.Format("btn{0}", i), true).FirstOrDefault();
                if (i == currentId)
                {
                    txt.BackColor = btn.BackColor;
                    txtItems.BackColor = btn.BackColor;
                }
                else
                {
        ...
        private void CalculateAmount(int deletedrowindex)
        {
            if (ActiveDataGrid == null)
                return;
            for (int counterId = 1; counterId <= 6; counterId++)
            {
                int ccid = Convert.ToInt32(ActiveDataGrid.MainDataGridCurrentRow.Cells["Col_CounterID"].Value);
                if (counterId == ccid)
                {
                    bool counterExist = false;
                    lblFooterMessage.Text = "";
                    double mTotalAmount = 0;
                    // double mTotalAmountforDiscount = 0;
                    double mTotalAmountVAT5 = 0;
                        ...
                        if (ActiveDataGrid != null)
                        {
                            ...
                            if (counterExist)
                            {
                                ...
                                if (counterId.ToString().Trim() == "1")
                                {
                                    ...
                                    txtamount1.Text = mTotalAmount.ToString("#0.00");
                                    txtitems1.Text = itemCount.ToString();
                                    txtCounter1.Text = mtotalafterdiscount.ToString("#0.00");
                                    txtBillReferenceAmt.Text = mTotalAmount.ToString("#0.00");
                                    if (cbRound.Checked == true)
                                    {
                                        if (General.CurrentSetting.MsetSaleRoundingToPreviousRupee == "Y")
                                        {
                                        ...
                                        txtCounter1.Text = Math.Round((Convert.ToDouble(txtCounter1.Text.ToString()) + Convert.ToDouble(txtRoundAmount.Text.ToString()) + mtotalmyspecialdiscamt12point5 + mtotalmyspecialdiscamt5), 2).ToString("#0.00");
                                    }
                                    else
                                    {
                                        txtRoundAmount.Text = "0.00";
                                        txtCounter1.Text = Math.Round((Convert.ToDouble(txtCounter1.Text.ToString()) + Convert.ToDouble(txtRoundAmount.Text.ToString()) + mdblMyDiscAmount), 2).ToString("#0.00");
                                    }
                                }
                                else if (counterId.ToString().Trim() == "2")
                                {
                                    txtamount2.Text = mTotalAmount.ToString("#0.00");
                                    txtitems2.Text = itemCount.ToString();
                                    txtCounter2.Text = mtotalafterdiscount.ToString("#0.00");
                                    txtBillReferenceAmt.Text = mTotalAmount.ToString("#0.00");
                                    if (cbRound.Checked == true)
                                    {
                                        if (General.CurrentSetting.MsetSaleRoundingToPreviousRupee == "Y")
                                        ...
                                        txtCounter2.Text = Math.Round((Convert.ToDouble(txtCounter2.Text.ToString()) + Convert.ToDouble(txtRoundAmount.Text.ToString()) + mtotalmyspecialdiscamt12point5 + mtotalmyspecialdiscamt5), 2).ToString("#0.00");
                                    }
                                    else
                                    {
                                        txtRoundAmount.Text = "0.00";
                                        txtCounter2.Text = Math.Round((Convert.ToDouble(txtCounter2.Text.ToString()) + Convert.ToDouble(txtRoundAmount.Text.ToString()) + mdblMyDiscAmount), 2).ToString("#0.00");
                                    }
                                }
                                else if (counterId.ToString().Trim() == "3")
                                {
                                    txtamount3.Text = mTotalAmount.ToString("#0.00");
                                    txtitems3.Text = itemCount.ToString();
                                    txtCounter3.Text = mtotalafterdiscount.ToString("#0.00");
                                    txtBillReferenceAmt.Text = mTotalAmount.ToString("#0.00");
                                    if (cbRound.Checked == true)
                                    {
                                        if (General.CurrentSetting.MsetSaleRoundingToPreviousRupee == "Y")
                                        ...
                                        txtCounter3.Text = Math.Round((Convert.ToDouble(txtCounter3.Text.ToString()) + Convert.ToDouble(txtRoundAmount.Text.ToString()) + mtotalmyspecialdiscamt12point5 + mtotalmyspecialdiscamt5), 2).ToString("#0.00");
                                    }
                                    else
                                    {
                                        txtRoundAmount.Text = "0.00";
                                        txtCounter3.Text = Math.Round((Convert.ToDouble(txtCounter3.Text.ToString()) + Convert.ToDouble(txtRoundAmount.Text.ToString()) + mdblMyDiscAmount), 2).ToString("#0.00");
                                    }
                                }
                                else if (counterId.ToString().Trim() == "4")
                                {
                                    txtamount4.Text = mTotalAmount.ToString("#0.00");
                                    txtitems4.Text = itemCount.ToString();
                                    txtCounter4.Text = mtotalafterdiscount.ToString("#0.00");
                                    txtBillReferenceAmt.Text = mTotalAmount.ToString("#0.00");
                                    if (cbRound.Checked == true)
                                    {
                                        if (General.CurrentSetting.MsetSaleRoundingToPreviousRupee == "Y")
                                        ...
                                        txtCounter4.Text = Math.Round((Convert.ToDouble(txtCounter4.Text.ToString()) + Convert.ToDouble(txtRoundAmount.Text.ToString()) + mtotalmyspecialdiscamt12point5 + mtotalmyspecialdiscamt5), 2).ToString("#0.00");
                                    }
                                    else
                                    {
                                        txtRoundAmount.Text = "0.00";
                                        txtCounter4.Text = Math.Round((Convert.ToDouble(txtCounter4.Text.ToString()) + Convert.ToDouble(txtRoundAmount.Text.ToString()) + mdblMyDiscAmount), 2).ToString("#0.00");
                                    }
                                }
                                else if (counterId.ToString().Trim() == "5")
                                {
                                    if (mTotalAmount != 0)
                                    {
                                        txtamount5.Text = mTotalAmount.ToString("#0.00");
                                        txtitems5.Text = itemCount.ToString();
                                        txtCounter5.Text = mtotalafterdiscount.ToString("#0.00");
                                        txtBillReferenceAmt.Text = mTotalAmount.ToString("#0.00");
                                        if (cbRound.Checked == true)
                                        {
                                            if (General.CurrentSetting.MsetSaleRoundingToPreviousRupee == "Y")
                                            ...
                                            txtCounter5.Text = Math.Round((Convert.ToDouble(txtCounter5.Text.ToString()) + Convert.ToDouble(txtRoundAmount.Text.ToString()) + mtotalmyspecialdiscamt12point5 + mtotalmyspecialdiscamt5), 2).ToString("#0.00");
                                        }
                                        else
                                        {
                                            txtRoundAmount.Text = "0.00";
                                            txtCounter5.Text = Math.Round((Convert.ToDouble(txtCounter5.Text.ToString()) + Convert.ToDouble(txtRoundAmount.Text.ToString()) + mdblMyDiscAmount), 2).ToString("#0.00");
                                        }
                                    }
                                }
                                else
                                {
                                    if (mTotalAmount != 0)
                                    {
                                        txtamount6.Text = mTotalAmount.ToString("#0.00");
                                        txtitems6.Text = itemCount.ToString();
                                        txtCounter6.Text = mtotalafterdiscount.ToString("#0.00");
                                        txtBillReferenceAmt.Text = mTotalAmount.ToString("#0.00");
                                        if (cbRound.Checked == true)
                                        {
                                            if (General.CurrentSetting.MsetSaleRoundingToPreviousRupee == "Y")
                                            ...
                                            txtCounter6.Text = Math.Round((Convert.ToDouble(txtCounter6.Text.ToString()) + Convert.ToDouble(txtRoundAmount.Text.ToString()) + mtotalmyspecialdiscamt12point5 + mtotalmyspecialdiscamt5), 2).ToString("#0.00");
                                        }
                                        else
                                        {
                                            txtRoundAmount.Text = "0.00";
                                            txtCounter6.Text = Math.Round((Convert.ToDouble(txtCounter6.Text.ToString()) + Convert.ToDouble(txtRoundAmount.Text.ToString()) + mdblMyDiscAmount), 2).ToString("#0.00");
                                        }
                                    }
                                }
            ...
            int counterID = Convert.ToInt32(ActiveDataGrid.MainDataGridCurrentRow.Cells["Col_CounterID"].Value);
            if (counterID == 1)
            {
                txtBillAmount.Text = txtBillReferenceAmt.Text;
                txtBillAmount2.Text = txtCounter1.Text;
            ...
            {
                txtBillAmount.Text = txtBillReferenceAmt.Text;
                txtBillAmount2.Text = txtCounter2.Text;
                txtNetAmount.Text = txtBillAmount2.Text;
                PanelRs.BackColor = btn2.BackColor;
            }
            else if (counterID == 3)
            {
                txtBillAmount.Text = txtBillReferenceAmt.Text;
                txtBillAmount2.Text = txtCounter3.Text;
                txtNetAmount.Text = txtBillAmount2.Text;
                PanelRs.BackColor = btn3.BackColor;
            }
            else if (counterID == 4)
            {
                txtBillAmount.Text = txtBillReferenceAmt.Text;
                txtBillAmount2.Text = txtCounter4.Text;
                txtNetAmount.Text = txtBillAmount2.Text;
                PanelRs.BackColor = btn4.BackColor;
            }
            else if (counterID == 5)
            {
                txtBillAmount.Text = txtBillReferenceAmt.Text;
                txtBillAmount2.Text = txtCounter5.Text;
                txtNetAmount.Text = txtBillAmount2.Text;
                PanelRs.BackColor = btn5.BackColor;
            }
            else
            {
                txtBillAmount.Text = txtBillReferenceAmt.Text;
                txtBillAmount2.Text = txtCounter6.Text;
                txtNetAmount.Text = txtBillAmount2.Text;
                PanelRs.BackColor = btn6.BackColor;
            }
        ...
        private void ChangeBackColour(PSProductViewCounter2Control activeDataGrid)
        {
        ...
        private void txtDiscPercent_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    lblFooterMessage.Text = "";
                    double TotalAmt = Convert.ToDouble(txtBillAmount.Text);
                    CalculateAmount(-1);
                    txtBillAmount.Text = TotalAmt.ToString();
                    txtNetAmount.Text = txtBillAmount2.Text;
                    txtNetAmount.Text = String.Format(new CultureInfo("en-IN"), "{0 :C2}", 123.45);
        ...
        private void txtDiscAmount_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                if (!string.IsNullOrEmpty(txtDiscAmount.Text))
                {
                    ...
                    {
                        mdiscperCalculated = (Convert.ToDouble(txtDiscAmount.Text) * 100) / Convert.ToDouble(txtBillAmount.Text);
...
try
{
        ...
        private void ConstructMainColumnsmpPVC(DataGridView activeDataGrid)
        {
            ...
            try
            {
                ...
                //0 New column added by Amit for Counter ID 
                column = new DataGridViewTextBoxColumn();
                column.Name = "Col_CounterID";
                column.DataPropertyName = "CounterID";
                column.HeaderText = "CounterID";
                column.Width = 30;
        ...
        private void ConstructMainColumns(PSProductViewCounter2Control activeDataGrid)
        {
            DataGridViewTextBoxColumn column;
            activeDataGrid.ColumnsMain.Clear();
            //0
        ...
        private void ConstructProductSelectionListGridColumns(PSProductViewCounter2Control activeDataGrid)
        {
            DataGridViewTextBoxColumn column;
        ...
        private void ConstructBatchGridColumns(PSProductViewCounter2Control activeDataGrid)
        {
            DataGridViewTextBoxColumn column;
        ...
        private void ActiveDataGridOnNewBatchClicked()
        {
            try
            {
                ...
                ActiveDataGrid.MainDataGridCurrentRow.Cells["Col_Quantity"].ReadOnly = false;
                ActiveDataGrid.SetFocus(7); //focus to Batch - 1
            }
            catch (Exception Ex)
            {
                Log.WriteException(Ex);
        ...
        private void ActiveGrid_OnArrowUpDownPressed(int keyValue)
        {
            try
            {
                int Rowindex = ActiveGrid.MainDataGridCurrentRow.Index;
            }
            catch { }
        }
